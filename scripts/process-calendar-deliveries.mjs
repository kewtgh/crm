...
async function rpc(name,body){return workerJson(`/db/rpc/${name}`,{method:"POST",body:JSON.stringify(body)});}
async function row(table,select,id){const result=await workerJson(`/db/table/${table}?select=${select}&id=eq.${id}&limit=1`);if(!result?.[0])throw new Error(`${table} record is unavailable`);return result[0];}
try{
