...
const jobs = await rpc("claim_notification_outbox", { batch_size:Number(process.env.OUTBOX_BATCH_SIZE ?? 20) });
let sent = 0;
for (const job of jobs) {
  try {
    ...
    await rpc("complete_notification_outbox", { job_id:job.id });
    sent += 1;
  } catch (error) {
    await rpc("fail_notification_outbox", { job_id:job.id,failure:error instanceof Error?error.message:"Unknown delivery error" });
  }
}
