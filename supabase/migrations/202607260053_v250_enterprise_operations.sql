...
create policy "administrators read enterprise directory"
  on public.enterprise_directory_users for select to authenticated
...
grant select,insert,update on public.enterprise_directory_users to service_role;
revoke delete on public.enterprise_directory_users from service_role;
revoke insert,update,delete on public.enterprise_directory_users from authenticated;
...
create policy "administrators read connector validation receipts"
  on public.connector_validation_receipts for select to authenticated
  using(public.is_workspace_member(workspace_id)
    and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));
grant select on public.connector_validation_receipts to authenticated;
grant select,insert on public.connector_validation_receipts to service_role;
revoke insert,update,delete on public.connector_validation_receipts from authenticated;
