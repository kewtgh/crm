...
function ActionForm({action,pending,error,paymentTime,directAuthority,t,onClose,onSubmit}:{action:{kind:ActionKind;id:string};pending:boolean;error:string;paymentTime:string;directAuthority:boolean;t:(key:string)=>string;onClose:()=>void;onSubmit:(event:React.FormEvent<HTMLFormElement>)=>void}){const direct=directAuthority&&(action.kind==="submit"||action.kind==="refund");return <form className="surface inline-action-panel" onSubmit={onSubmit}><div><b>{t(direct?`finance.action.${action.kind}Direct`:`finance.action.${action.kind}`)}</b><button type="button" className="icon-button" onClick={onClose} aria-label={t("common.close")}>×</button></div>{direct&&<InlineMessage type="info">{t("finance.directAuthorityHelp")}</InlineMessage>}{action.kind==="accept"&&<InlineMessage type="warning">{t("finance.acceptConfirm")}</InlineMessage>}{action.kind==="submit"&&<TextArea label={t("finance.reason")} name="reason"/>}{action.kind==="convert"&&<div className="form-grid three-column"><Field label={t("finance.contractNumber")} name="contractNo"/><Field label={t("finance.start")} name="start" type="date"/><Field label={t("finance.end")} name="end" type="date"/></div>}{action.kind==="schedule"&&<TextArea label={t("finance.installments")} name="installments" placeholder="2026-08-01:50000; 2026-10-01:50000"/>}{action.kind==="payment"&&<div className="form-grid three-column"><Field label={t("finance.amount")} name="amount" type="number"/><Field label={t("finance.reference")} name="reference"/><Field label={t("finance.paidOn")} name="paidOn" type="datetime-local" defaultValue={paymentTime}/></div>}{action.kind==="refund"&&<div className="form-grid two-column"><Field label={t("finance.amount")} name="amount" type="number"/><Field label={t("finance.reason")} name="reason"/></div>}{action.kind==="completeRefund"&&<Field label={t("finance.receipt")} name="receipt"/>}{error&&<InlineMessage type="error">{error}</InlineMessage>}<button className="primary-button" disabled={pending}><CheckCircle2 size={16}/>{pending?t("common.saving"):t("common.confirm")}</button></form>}
