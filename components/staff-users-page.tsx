...
function TeamDialog({open,teams,leadCandidates,close,onSaved}:{open:boolean;teams:TeamRecord[];leadCandidates:TeamLeadCandidate[];close:()=>void;onSaved:(team:TeamRecord)=>void}){
  ...
  return <dialog className="staff-dialog" ref={dialogRef} onCancel={event=>{if(pending)event.preventDefault();else close();}}>
    ...
    <form key={editingId||"new"} className="staff-invite-form" onSubmit={submit}><div className="auth-form-heading"><p className="eyebrow">{t("admin.teams.eyebrow")}</p><h2>{t(editing?"admin.teams.edit":"admin.teams.new")}</h2><p>{t("admin.teams.help")}</p></div>
      ...
      <label className="field"><span>{t("admin.teams.lead")}</span><select name="leadUserId" defaultValue={editing?.leadUserId??""}><option value="">{t("admin.teams.leadLater")}</option>{leadCandidates.map(candidate=><option key={candidate.memberId} value={candidate.userId}>{candidate.name} · {t(roleMessageKey[candidate.role as AppRole])}</option>)}</select></label>
