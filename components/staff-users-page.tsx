...
export function StaffUsersPage({ initialItems, initialTotal }: { initialItems: StaffUserRecord[]; initialTotal: number }) {
  ...
  const [teamEditor,setTeamEditor]=useState<{mode:"create"}|{mode:"edit";team:TeamRecord}|null>(null);
  const [teamDetail,setTeamDetail]=useState<TeamRecord|null>(null);
  ...
  return <div className="page-stack">
    ...
    <section className="surface team-management-panel"><div className="surface-heading"><div><p className="eyebrow">{t("admin.teams.eyebrow")}</p><h2>{t("admin.teams.managementTitle")}</h2><p>{t("admin.teams.managementHelp")}</p></div><StatusBadge tone={teams.some(team=>team.members.some(member=>member.status==="PENDING"))?"amber":"blue"}>{t("admin.teams.existingCount",{count:teams.length})}</StatusBadge></div><div className="team-management-grid">{teams.map(team=><article className="team-management-card" key={team.id}><div className="team-card-heading"><span><b>{team.nameZh} / {team.nameEn}</b><small>{team.code}</small></span><StatusBadge tone={team.active?"green":"gray"}>{t(team.active?"common.enabled":"common.inactive")}</StatusBadge></div><div className="team-card-metrics"><span><b>{team.memberCount}</b><small>{t("admin.teams.members")}</small></span><span><b>{team.leadNames.length}</b><small>{t("admin.teams.leads")}</small></span><span><b>{team.members.filter(member=>member.status==="PENDING").length}</b><small>{t("admin.teams.pendingRequests")}</small></span></div><p>{team.leadNames.length?team.leadNames.join("、"):t("admin.teams.noLeads")}</p><div className="team-card-actions"><button className="secondary-button compact" type="button" onClick={()=>setTeamDetail(team)}><Eye size={15}/>{t("common.details")}</button><button className="primary-button compact" type="button" onClick={()=>setTeamEditor({mode:"edit",team})}><Pencil size={15}/>{t("common.edit")}</button></div></article>)}</div>{!teams.length&&<div className="empty-state"><span>{t("admin.teams.none")}</span><button className="primary-button" type="button" onClick={()=>setTeamEditor({mode:"create"})}><Plus size={16}/>{t("admin.teams.new")}</button></div>}</section>
    ...
    <TeamDialog open={teamEditor!==null} team={teamEditor?.mode==="edit"?teamEditor.team:null} leadCandidates={leadCandidates} close={()=>setTeamEditor(null)} onSaved={team=>{setTeams(current=>[...current.filter(item=>item.id!==team.id),team].sort((a,b)=>a.nameEn.localeCompare(b.nameEn)));setTeamEditor(null);setToast(t("admin.teams.saved"));}}/>
    {teamDetail&&<AccessibleDrawer title={`${teamDetail.nameZh} / ${teamDetail.nameEn}`} eyebrow={t("admin.teams.eyebrow")} description={`${teamDetail.code} · ${t("admin.teams.existingCount",{count:teams.length})}`} onClose={()=>setTeamDetail(null)}><div className="team-detail"><div className="team-detail-summary"><StatusBadge tone={teamDetail.active?"green":"gray"}>{t(teamDetail.active?"common.enabled":"common.inactive")}</StatusBadge><span><b>{t("admin.teams.leads")}</b><small>{teamDetail.leadNames.length?teamDetail.leadNames.join("、"):t("admin.teams.noLeads")}</small></span></div><section><h3>{t("admin.teams.description")}</h3><MarkdownContent value={teamDetail.descriptionMarkdown} empty={t("admin.teams.descriptionEmpty")}/></section><section><h3>{t("admin.teams.members")}</h3><div className="team-detail-members">{teamDetail.members.map(member=><article key={member.memberId}><span><b>{member.nameZh} / {member.nameEn}</b><small>{t(roleMessageKey[member.role as AppRole])}</small></span><StatusBadge tone={member.status==="PENDING"?"amber":member.membershipRole==="LEAD"?"purple":"green"}>{t(member.status==="PENDING"?"admin.teams.membership.pending":member.membershipRole==="LEAD"?"admin.teams.leadBadge":"admin.teams.membership.active")}</StatusBadge></article>)}</div>{!teamDetail.members.length&&<p className="select-empty">{t("admin.teams.noMembers")}</p>}</section><div className="drawer-actions"><button className="primary-button" type="button" onClick={()=>{setTeamEditor({mode:"edit",team:teamDetail});setTeamDetail(null);}}><Pencil size={16}/>{t("common.edit")}</button></div></div></AccessibleDrawer>}
...
function StaffActionMenu({item,currentUser,open,setOpen,onTeam,onStatus,onResend}:{item:StaffUserRecord;currentUser:ReturnType<typeof useAppUser>;open:boolean;setOpen:React.Dispatch<React.SetStateAction<string|null>>;onTeam:()=>void;onStatus:()=>void;onResend:()=>void}){
  const{t}=useI18n();
  ...
  return <div className="staff-action-menu" data-staff-action-menu={item.id}><button ref={triggerRef} className="icon-button staff-action-trigger" type="button" aria-label={t("admin.users.accountActions",{name:item.displayNameEn})} aria-haspopup="menu" aria-expanded={open} aria-controls={open?`staff-actions-${item.id}`:undefined} onClick={()=>setOpen((current)=>current===item.id?null:item.id)}><MoreHorizontal size={18}/></button>{open&&<div ref={menuRef} id={`staff-actions-${item.id}`} role="menu" onKeyDown={onKeyDown}><button type="button" role="menuitem" onClick={()=>{setOpen(null);onTeam();}}><Users size={15}/>{t("admin.teams.adjustMemberships")}</button><button type="button" role="menuitem" disabled={protectedAccount||item.id===currentUser.id} onClick={()=>{setOpen(null);onStatus();}}>{t(item.status==="ACTIVE"?"admin.users.suspendAction":"admin.users.activateAction",{name:item.displayNameEn})}</button>{awaitingConfirmation&&<button type="button" role="menuitem" onClick={()=>{setOpen(null);onResend();}}><RotateCcw size={15}/>{t("admin.users.resendInvitation")}</button>}</div>}</div>;
}
...
function TeamDialog({open,team,leadCandidates,close,onSaved}:{open:boolean;team:TeamRecord|null;leadCandidates:TeamLeadCandidate[];close:()=>void;onSaved:(team:TeamRecord)=>void}){
  const{t}=useI18n();const dialogRef=useRef<HTMLDialogElement>(null);const[pending,setPending]=useState(false);const[error,setError]=useState("");
  useEffect(()=>{const dialog=dialogRef.current;if(open&&!dialog?.open)dialog?.showModal();if(!open&&dialog?.open)dialog.close();},[open]);
  const closeDialog=()=>{if(pending)return;setError("");close();};
  const submit=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);setPending(true);setError("");try{const result=await apiFetch<{item:TeamRecord}>("/api/admin/teams",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({id:team?.id,code:form.get("code"),nameZh:form.get("nameZh"),nameEn:form.get("nameEn"),descriptionMarkdown:form.get("descriptionMarkdown"),leadUserIds:form.getAll("leadUserIds"),active:form.get("active")==="on"})});onSaved(result.item);}catch{setError(t("admin.teams.saveFailed"));}finally{setPending(false);}};
  return <dialog className="staff-dialog" ref={dialogRef} onClose={closeDialog} onCancel={event=>{if(pending)event.preventDefault();else closeDialog();}}>
    ...
    <form key={team?.id??"new"} className="staff-invite-form" onSubmit={submit}><div className="auth-form-heading"><p className="eyebrow">{t("admin.teams.eyebrow")}</p><h2>{t(team?"admin.teams.edit":"admin.teams.new")}</h2><p>{t("admin.teams.help")}</p></div>
      ...
      <label className="checkbox-row"><input name="active" type="checkbox" defaultChecked={team?.active??true}/><span>{t("admin.teams.activeHelp")}</span></label>
      {error&&<InlineMessage type="error">{error}</InlineMessage>}<div className="drawer-actions"><button className="secondary-button" type="button" disabled={pending} onClick={closeDialog}>{t("common.cancel")}</button><button className="primary-button" disabled={pending}><Plus size={16}/>{pending?t("common.saving"):t(team?"common.save":"common.create")}</button></div>
    </form></dialog>;
}
