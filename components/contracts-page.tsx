...
import { useAppUser, useCapability } from "@/components/app-user-context";
...
export function ContractsPage({ initialContracts = [], initialTotal = 0, initialSummary, persistent = false }: { initialContracts?: Contract[]; initialTotal?: number; initialSummary:ContractSummary; persistent?: boolean }) {
  ...
  const requestApproval=async(type:"CONTRACT_SIGN"|"CONTRACT_EXPORT")=>{setFlowError("");if(!selectedId){setFlowError(t("flow.selectContract"));return;}setApprovalPending(true);try{const result=await apiFetch<{direct?:boolean}>("/api/approvals",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({type,objectType:"CONTRACT",objectId:selectedId,reason:t(type==="CONTRACT_SIGN"?"approval.reason.sign":"approval.reason.export")})});setToast(t(result.direct?"flow.directExecuted":"flow.approvalRequested"));if(result.direct)await load(page);}catch(caught){setFlowError(presentApiError(caught,t,"flow.approvalFailed").message);}finally{setApprovalPending(false);}};
  ...
  const createContract=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();setFormError("");if(!organization){setFormError(t("contracts.organizationRequired"));return;}const form=new FormData(event.currentTarget);setCreatePending(true);try{await apiFetch("/api/contracts",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({operation:"create",contractNumber:String(form.get("contractNumber")),organizationId:organization,productId:product||null,startDate:String(form.get("startDate")),endDate:String(form.get("endDate")),currency:String(form.get("currency")).toUpperCase(),value:Number(form.get("value")),relationshipLevel:Number(form.get("relationshipLevel"))})});setCreateOpen(false);await load(1);setToast(t("contracts.created"));}catch(caught){setFormError(presentApiError(caught,t,"contracts.createFailed").message);}finally{setCreatePending(false);}};
