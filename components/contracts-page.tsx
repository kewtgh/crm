...
export function ContractsPage({ initialContracts = [], initialTotal = 0, initialSummary, persistent = false }: { initialContracts?: Contract[]; initialTotal?: number; initialSummary:ContractSummary; persistent?: boolean }) {
  ...
  const load=useCallback(async(nextPage:number)=>{setLoading(true);try{const result=await apiFetch<{items:Contract[];total?:number;summary:ContractSummary}>(`/api/contracts?page=${nextPage}&pageSize=${pageSize}&status=${encodeURIComponent(status)}&query=${encodeURIComponent(query)}`);setRecords(result.items);setTotal(result.total??result.items.length);setSummary(result.summary);}catch{setFlowError(t("contracts.loadFailed"));}finally{setLoading(false);}},[pageSize,query,status,t]);
  useEffect(()=>{if(!persistent)return;const timer=setTimeout(()=>void load(page),250);return()=>clearTimeout(timer);},[load,page,persistent]);
  useEffect(()=>{if(!createOpen)return;void apiFetch<{items?:Array<{id:string;nameZh:string;nameEn:string;active:boolean}>}>("/api/products").then(result=>setProductOptions((result.items??[]).filter(item=>item.active).map(item=>({value:item.id,label:locale==="zh-CN"?item.nameZh:item.nameEn})))).catch(()=>setFormError(t("contracts.productsLoadFailed")));},[createOpen,locale,t]);
  ...
  const requestApproval=async(type:"CONTRACT_SIGN"|"CONTRACT_EXPORT")=>{setFlowError("");if(!selectedId){setFlowError(t("flow.selectContract"));return;}setApprovalPending(true);try{await apiFetch("/api/approvals",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({type,objectType:"CONTRACT",objectId:selectedId,reason:t(type==="CONTRACT_SIGN"?"approval.reason.sign":"approval.reason.export")})});setToast(t("flow.approvalRequested"));}catch{setFlowError(t("flow.approvalFailed"));}finally{setApprovalPending(false);}};
  const searchOrganizations=async(value:string)=>{setOrganizationLoading(true);try{const result=await apiFetch<{items?:Array<{value:string;labelZh:string;labelEn:string;type:string}>}>(`/api/search/related?q=${encodeURIComponent(value)}`);setOrganizationOptions((result.items??[]).filter(item=>item.type==="ORGANIZATION").map(item=>({value:item.value.split(":")[1],label:locale==="zh-CN"?item.labelZh:item.labelEn,detail:t("contracts.customer")})));}catch{setFormError(t("contracts.organizationRequired"));}finally{setOrganizationLoading(false);}};
  const createContract=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();setFormError("");if(!organization){setFormError(t("contracts.organizationRequired"));return;}const form=new FormData(event.currentTarget);try{await apiFetch("/api/contracts",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({operation:"create",contractNumber:String(form.get("contractNumber")),organizationId:organization,productId:product||null,startDate:String(form.get("startDate")),endDate:String(form.get("endDate")),currency:String(form.get("currency")).toUpperCase(),value:Number(form.get("value")),relationshipLevel:Number(form.get("relationshipLevel"))})});}catch{setFormError(t("contracts.createFailed"));return;}setCreateOpen(false);await load(1);setToast(t("contracts.created"));};
  const prepareRenewal=async(contract:Contract)=>{setRenewalPending(contract.id);setFlowError("");try{await apiFetch("/api/contracts",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({operation:"renewal",id:contract.id})});}catch{setFlowError(t("contracts.renewalFailed"));setRenewalPending("");return;}setRenewalPending("");await load(page);setToast(t("contracts.renewalCreated",{name:locale==="zh-CN"?contract.customer:contract.english}));};
  const openPlaybook=async()=>{if(!selectedId){setFlowError(t("contracts.playbookSelect"));return;}setPlaybookLoading(true);setFormError("");try{const result=await apiFetch<{context:RenewalPlaybookContext}>(`/api/contracts/${selectedId}/renewal-playbook`);setPlaybookContext(result.context);setPlaybookOpen(true);}catch{setFlowError(t("contracts.playbookLoadFailed"));}finally{setPlaybookLoading(false);}};
  const savePlaybook=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();if(!selectedId)return;setPlaybookSaving(true);setFormError("");const form=new FormData(event.currentTarget);try{await apiFetch(`/api/contracts/${selectedId}/renewal-playbook`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({stage:form.get("stage"),risk:form.get("risk"),actionZh:form.get("actionZh"),actionEn:form.get("actionEn"),dueAt:localDateTimeToIso(String(form.get("dueAt"))),outcome:form.get("outcome")})});setPlaybookOpen(false);setToast(t("contracts.playbookSaved"));}catch{setFormError(t("contracts.playbookFailed"));}finally{setPlaybookSaving(false);}};
  ...
  return <div className="page-stack contracts-page">
    <section className="page-heading-row"><div><p className="eyebrow">{t("contracts.eyebrow")}</p><h1>{t("contracts.title")}</h1><p>{t("contracts.description")}</p></div><div className="page-actions"><button className="secondary-button" type="button" onClick={()=>{setCreateOpen(true);setFormError("");}}><Plus size={17}/>{t("contracts.new")}</button><button className="secondary-button" type="button" disabled={!selectedId||playbookLoading} onClick={()=>void openPlaybook()}><RefreshCcw className={playbookLoading?"spin":""} size={17}/>{t("contracts.playbook")}</button><button className="secondary-button" type="button" disabled={approvalPending} onClick={() => requestApproval("CONTRACT_EXPORT")}><FileDown size={17} />{t("flow.requestExportApproval")}</button><button className="primary-button" type="button" disabled={approvalPending} onClick={() => requestApproval("CONTRACT_SIGN")}><Signature size={17} />{t("flow.requestSignApproval")}</button></div></section>
