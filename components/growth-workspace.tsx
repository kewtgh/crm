...
export function GrowthWorkspace({ initial }: { initial: GrowthSnapshot }) {
  ...
  return <div className="page-stack v220-workspace">
    ...
    {drawer && <AccessibleDrawer pending={pending} title={t(`growth.drawer.${drawer}`)} onClose={() => { setDrawer(null); setError(""); }}>
      <form onSubmit={submit}>
        ...
        <div className="drawer-actions"><button type="button" className="secondary-button" disabled={pending} onClick={() => setDrawer(null)}>{t("common.cancel")}</button><button className="primary-button" disabled={pending || ((drawer === "attribution" || drawer === "journey") && !lead)}>{pending ? t("common.saving") : t("common.save")}</button></div>
      </form>
    </AccessibleDrawer>}
