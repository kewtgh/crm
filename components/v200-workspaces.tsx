...
import { AccessibleDrawer, ConfirmDialog, InlineMessage, Pagination, SearchableSelect, SearchField, StatusBadge, Toast } from "./ui";
...
const statusTone = (status: string) => status === "ACTIVE" || status === "APPLIED" || status === "ACCEPTED" || status === "FULFILLED" || status === "QUALIFIED"
  ? "green" : status === "REJECTED" || status.includes("FAILED") || status === "DISQUALIFIED" ? "red" : "amber";

function MarkdownField({name,label,defaultValue=""}:{name:string;label:string;defaultValue?:string}){
  const{t}=useI18n();return <label className="field"><span>{label}</span><textarea name={name} rows={4} defaultValue={defaultValue} data-markdown="true"/><small>{t("common.markdownSupported")}</small></label>;
}
...
export function StudentsWorkspace({ initial, initialDetail = null }: { initial: PageResult<StudentRecord>; initialDetail?: StudentDetail | null }) {
  ...
  return <div className="page-stack v200-workspace">
    ...
    {open && <AccessibleDrawer pending={pending} title={t("education.newStudent")} onClose={() => setOpen(false)}><form onSubmit={create}>
      <SearchableSelect label={t("education.person")} value={person} options={people} onChange={setPerson} onSearch={searchPeople}/><SearchableSelect label={t("education.householdOptional")} value={household} options={households} onChange={setHousehold} onSearch={searchHouseholds}/>
      <div className="form-grid two-column"><label className="field"><span>{t("education.studentNumber")}</span><input name="studentNumber" maxLength={60}/></label><label className="field"><span>{t("education.birthDate")}</span><input name="birthDate" type="date"/></label></div>
      <div className="form-grid two-column"><label className="field"><span>{t("education.grade")}</span><input name="grade" required maxLength={40}/></label><label className="field"><span>{t("education.currentClass")}</span><input name="currentClass" maxLength={80}/></label></div>
      <div className="form-grid two-column"><label className="field"><span>{t("education.academicYear")}</span><input name="academicYear" placeholder="2026-2027" required/></label><label className="field"><span>{t("education.learningStyle")}</span><select name="preferredLearningStyle" defaultValue="UNSPECIFIED">{["UNSPECIFIED","VISUAL","AUDITORY","READ_WRITE","KINESTHETIC","MIXED"].map(value=><option value={value} key={value}>{t(`education.learningStyle.${value.toLowerCase()}`)}</option>)}</select></label></div>
      <label className="field"><span>{t("education.interests")}</span><input name="interests" placeholder={t("education.interestsHelp")}/></label>
    ...
    {detail && <AccessibleDrawer pending={pending} title={`${detail.nameZh} / ${detail.nameEn}`} description={t("education.detailHelp")} onClose={() => setDetail(null)}>
      <form onSubmit={saveStudent}>
        <div className="form-grid two-column"><label className="field"><span>{t("education.studentNumber")}</span><input name="studentNumber" defaultValue={detail.studentNumber}/></label><label className="field"><span>{t("education.birthDate")}</span><input name="birthDate" type="date" defaultValue={detail.birthDate}/></label></div>
        <div className="form-grid two-column"><label className="field"><span>{t("education.grade")}</span><input name="grade" defaultValue={detail.grade} required/></label><label className="field"><span>{t("education.currentClass")}</span><input name="currentClass" defaultValue={detail.currentClass}/></label></div>
        <div className="form-grid two-column"><label className="field"><span>{t("education.academicYear")}</span><input name="academicYear" defaultValue={detail.academicYear} required/></label><label className="field"><span>{t("education.learningStyle")}</span><select name="preferredLearningStyle" defaultValue={detail.preferredLearningStyle}>{["UNSPECIFIED","VISUAL","AUDITORY","READ_WRITE","KINESTHETIC","MIXED"].map(value=><option value={value} key={value}>{t(`education.learningStyle.${value.toLowerCase()}`)}</option>)}</select></label></div>
        <SearchableSelect label={t("education.householdOptional")} value={household} options={households} onChange={setHousehold} onSearch={searchHouseholds}/><label className="field"><span>{t("education.interests")}</span><input name="interests" defaultValue={detail.interests.join(", ")}/></label>
...
export function HouseholdsWorkspace({ initial, initialDetail = null }: { initial: PageResult<HouseholdRecord>; initialDetail?: HouseholdDetail | null }) {
  ...
  return <div className="page-stack v200-workspace"><section className="page-heading-row"><div><p className="eyebrow">{t("education.eyebrow")}</p><h1>{t("education.households")}</h1><p>{t("education.householdsHelp")}</p></div>{canManage && <button className="primary-button" onClick={() => setOpen(true)}><Plus size={17}/>{t("education.newHousehold")}</button>}</section>
    ...
    {open && <AccessibleDrawer pending={pending} title={t("education.newHousehold")} onClose={() => setOpen(false)}><form onSubmit={create}>
      <div className="form-grid two-column"><label className="field"><span>{t("education.nameZh")}</span><input name="nameZh" required/></label><label className="field"><span>{t("education.nameEn")}</span><input name="nameEn" required/></label></div>
      ...
      <div className="form-grid two-column"><label className="field"><span>{t("education.annualIncome")}</span><input name="annualIncomeAmount" type="number" min="0" step="1000"/></label><label className="field"><span>{t("education.incomeCurrency")}</span><input name="incomeCurrency" defaultValue="CNY" pattern="[A-Za-z]{3}" required/></label></div>
      <div className="form-grid two-column"><label className="field"><span>{t("education.preferredContactMethod")}</span><select name="preferredContactMethod" defaultValue="EMAIL">{["EMAIL","PHONE","SMS","WECHAT","WHATSAPP","IN_PERSON"].map(value=><option key={value} value={value}>{t(`education.contactMethod.${value.toLowerCase()}`)}</option>)}</select></label><label className="field"><span>{t("education.preferredLanguage")}</span><input name="preferredLanguage"/></label></div>
    ...
    {detail && <AccessibleDrawer pending={pending} title={`${detail.nameZh} / ${detail.nameEn}`} description={t("education.householdDetailHelp")} onClose={() => setDetail(null)}>
      <form onSubmit={saveHousehold}>
        <div className="form-grid two-column"><label className="field"><span>{t("education.nameZh")}</span><input name="nameZh" defaultValue={detail.nameZh} required/></label><label className="field"><span>{t("education.nameEn")}</span><input name="nameEn" defaultValue={detail.nameEn} required/></label></div>
        ...
        <div className="form-grid two-column"><label className="field"><span>{t("education.annualIncome")}</span><input name="annualIncomeAmount" type="number" min="0" step="1000" defaultValue={detail.annualIncomeAmount??""}/></label><label className="field"><span>{t("education.incomeCurrency")}</span><input name="incomeCurrency" defaultValue={detail.incomeCurrency} pattern="[A-Za-z]{3}" required/></label></div>
        <div className="form-grid two-column"><label className="field"><span>{t("education.preferredContactMethod")}</span><select name="preferredContactMethod" defaultValue={detail.preferredContactMethod}>{["EMAIL","PHONE","SMS","WECHAT","WHATSAPP","IN_PERSON"].map(value=><option key={value} value={value}>{t(`education.contactMethod.${value.toLowerCase()}`)}</option>)}</select></label><label className="field"><span>{t("education.preferredLanguage")}</span><input name="preferredLanguage" defaultValue={detail.preferredLanguage}/></label></div>
        ...
        <label className="field"><span>{t("common.status")}</span><select name="status" defaultValue={detail.status}><option value="ACTIVE">{t("education.status.active")}</option><option value="INACTIVE">{t("education.status.inactive")}</option></select></label>{error && <InlineMessage type="error">{error}</InlineMessage>}{canManage && <div className="drawer-actions"><button className="danger-button" type="button" disabled={pending} onClick={() => setConfirmation({kind:"archive"})}>{t("education.archiveHousehold")}</button><button className="primary-button" disabled={pending}>{pending ? t("common.saving") : t("common.save")}</button></div>}
      </form>
...
export function ProgressionWorkspace({ initial }: { initial: PageResult<ProgressionBatchRecord> }) {
  ...
  const [cancelConfirmation,setCancelConfirmation]=useState<ProgressionBatchRecord|null>(null);
  ...
  const cancel=async(item:ProgressionBatchRecord)=>{
    ...
    catch(caught){setError(presentApiError(caught,t,"progression.failed").message);}finally{setPending(false);setCancelConfirmation(null);}
  };
