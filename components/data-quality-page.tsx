...
export function DataQualityPage({ initialItems, initialTotal }: { initialItems: QualityIssue[]; initialTotal: number }) {
  ...
  const load = async (nextPage = page) => {
    const response = await fetch(`/api/data-quality?page=${nextPage}&q=${encodeURIComponent(query)}`);
    const result = await response.json() as { items?: QualityIssue[]; total?: number };
  ...
  const runRules = async () => {
    setPending(true);
    setError("");
    const response = await fetch("/api/data-quality", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ operation: "run" }) });
    setPending(false);
    if (!response.ok) {
  ...
  return <div className="page-stack quality-page">
    ...
    <section className="surface quality-list">
      ...
      {items.map((item) => <article className="quality-row" key={item.id}>
        ...
        <div><b>{t(item.titleKey)}</b><small>{item.entityType} · {item.entityId.slice(0, 8)}</small><small>{Object.values(item.details).filter(Boolean).join(" / ")}</small></div>
