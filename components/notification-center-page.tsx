...
export function NotificationCenterPage({initialItems,initialTotal}:{initialItems:NotificationRecord[];initialTotal:number}){
  const {t}=useI18n();const {formatDate}=useUserPreferences();const [items,setItems]=useState(initialItems);const [total,setTotal]=useState(initialTotal);const [page,setPage]=useState(1);const [error,setError]=useState("");const pageSize=10;
  const load=async(next:number)=>{setError("");try{const result=await apiFetch<{items:NotificationRecord[];total:number}>(`/api/notifications?page=${next}&pageSize=${pageSize}`);setPage(next);setItems(result.items);setTotal(result.total);}catch{setError(t("nav.notification.loadFailed"));}};
