  ...
  return <section className="task-workspace">
    ...
    <div className="task-workspace-grid">
      <article className="surface task-work-list"><div className="surface-heading"><div><p className="eyebrow">{t("eyebrow.priorityQueue")}</p><h2>{t("tasks.workQueue")}</h2></div>{selected.length>0&&<button className="secondary-button" type="button" onClick={()=>setConfirmOpen(true)}>{t("tasks.bulkComplete")} · {selected.length}</button>}</div>
        ...
        {workspace.items.length>12&&<InlineMessage type="info">{t("tasks.queueTruncated",{shown:12,total:workspace.items.length})} <Link href="#record-list">{t("tasks.viewCompleteList")}</Link></InlineMessage>}
      </article>
    ...
    {confirmOpen&&<AccessibleDrawer pending={pending} title={t("tasks.bulkComplete")} description={t("common.actionCannotUndo")} onClose={()=>setConfirmOpen(false)}><form onSubmit={bulk}><InlineMessage type="warning">{t("modules.selected",{count:selected.length})}</InlineMessage><label className="field"><span>{t("finance.reason")}</span><textarea name="reason" minLength={3} maxLength={500} required/></label><div className="drawer-actions"><button className="secondary-button" type="button" disabled={pending} onClick={()=>setConfirmOpen(false)}>{t("common.cancel")}</button><button className="primary-button" disabled={pending}><Clock3 size={16}/>{pending?t("common.processing"):t("common.confirm")}</button></div></form></AccessibleDrawer>}
