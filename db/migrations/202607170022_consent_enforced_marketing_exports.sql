...
begin
  if app_auth.current_user_id() is null or ws is null or nullif(trim(business_reason),'') is null then raise exception 'approval_not_authorized'; end if;
  ...
  insert into public.approval_requests(workspace_id,request_number,request_type,business_object_type,business_object_id,requester_id,reason,expires_at) values(ws,next_number,request_kind,object_type,object_id,app_auth.current_user_id(),trim(business_reason),now()+interval '7 days') returning * into created;
  ...
  insert into public.approval_actions(approval_request_id,actor_id,action,comment) values(created.id,app_auth.current_user_id(),'SUBMITTED',trim(business_reason));return created;
...
revoke all on function public.request_marketing_contact_export(text,text),public.marketing_export_rows(uuid,text) from public,crm_system;
grant execute on function public.request_marketing_contact_export(text,text) to crm_app;
grant execute on function public.marketing_export_rows(uuid,text) to crm_system, crm_worker;
