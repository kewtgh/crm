...
begin
  ...
  return jsonb_build_object(
    ...
    'mfaLevel',coalesce(app_auth.current_claims()->>'aal','aal1'),'workspaceId',ws
  );
end;
$$;

revoke all on function public.workspace_relationship_health(),
  public.explain_record_access(text,uuid,text) from public,crm_system;
grant execute on function public.workspace_relationship_health(),
  public.explain_record_access(text,uuid,text) to crm_app;
...
revoke insert on public.crm_activities from crm_app;
...
create table if not exists public.contract_renewal_playbooks(
  ...
  owner_id uuid not null references app_auth.accounts(id),
  outcome_reason text not null default '',
  created_by uuid not null references app_auth.accounts(id),
  updated_by uuid not null references app_auth.accounts(id),
...
create policy "renewal playbooks follow contract scope"
  on public.contract_renewal_playbooks for select to crm_app
  using(exists(
...
begin
  ...
  insert into public.contract_renewal_playbooks(
    ...
    action_due,coalesce(contract.owner_id,app_auth.current_user_id()),trim(coalesce(outcome,'')),app_auth.current_user_id(),app_auth.current_user_id()
    ...
    outcome_reason=excluded.outcome_reason,updated_by=app_auth.current_user_id(),updated_at=now()
  returning * into result;
  return result;
end;
$$;

grant select on public.contract_renewal_playbooks to crm_app;
...
create table if not exists public.opportunity_stage_history(
  ...
  changed_by uuid not null references app_auth.accounts(id),
...
create policy "opportunity history follows opportunity scope"
  on public.opportunity_stage_history for select to crm_app
  using(exists(
...
revoke update on public.opportunities from crm_app;
grant select on public.opportunity_stage_history to crm_app;
...
begin
  ...
  insert into public.audit_events(
    ...
    public.current_workspace_id(),app_auth.current_user_id(),'duplicate_merge',target_record::text,'MERGE',
...
begin
  select * into batch from public.import_batches
    where id=target_batch and workspace_id=public.current_workspace_id()
      and (created_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));
...
revoke all on function public.duplicate_merge_preview(text,uuid,uuid),
  ...
  public.import_dry_run(uuid) from public,crm_system;
grant execute on function public.duplicate_merge_preview(text,uuid,uuid),
  public.merge_duplicate_records(text,uuid,uuid,jsonb),
  public.import_dry_run(uuid) to crm_app;
...
create policy "administrators read worker heartbeats"
  on public.worker_heartbeats for select to crm_app
...
grant select on public.worker_heartbeats to crm_app;
revoke all on function public.record_worker_heartbeat(text,boolean,text,jsonb) from public,crm_system,crm_app;
grant execute on function public.record_worker_heartbeat(text,boolean,text,jsonb) to crm_system, crm_worker;
revoke all on function public.operational_snapshot() from public,crm_system;
grant execute on function public.operational_snapshot() to crm_app;
revoke all on function public.operational_retryable_jobs() from public,crm_system;
grant execute on function public.operational_retryable_jobs() to crm_app;
revoke all on function public.retry_operational_job(text,uuid) from public,crm_system;
grant execute on function public.retry_operational_job(text,uuid) to crm_app;
...
create table if not exists public.integration_connections(
  ...
  last_error text,
  configured_by uuid references app_auth.accounts(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create policy "leaders read integrations" on public.integration_connections for select to crm_app
  using(public.is_workspace_member(workspace_id)
    and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));
create policy "administrators read webhook inbox" on public.webhook_inbox for select to crm_app
  using(public.is_workspace_member(workspace_id)
...
returns void language sql security definer set search_path=public,app_auth,extensions
...
grant select on public.integration_connections,public.webhook_inbox to crm_app;
...
create table if not exists public.exchange_rate_snapshots(
  ...
  locked_by uuid not null references app_auth.accounts(id),
  created_at timestamptz not null default now(),
...
create policy "members read product bundles" on public.product_bundles for select to crm_app
  using(public.is_workspace_member(workspace_id));
create policy "members read product bundle items" on public.product_bundle_items for select to crm_app
  using(exists(select 1 from public.product_bundles b where b.id=bundle_id and public.is_workspace_member(b.workspace_id)));
create policy "members read exchange rate snapshots" on public.exchange_rate_snapshots for select to crm_app
  using(public.is_workspace_member(workspace_id));
...
begin
  ...
  insert into public.product_bundles(
    ...
    ws,upper(trim(bundle_code)),trim(bundle_name_zh),trim(bundle_name_en),app_auth.current_user_id()
  ) returning * into result;
...
begin
  ...
  insert into public.exchange_rate_snapshots(
    ...
    trim(rate_source),effective,app_auth.current_user_id()
  ) returning * into result;
  return result;
end;
$$;

grant select on public.product_bundles,public.product_bundle_items to crm_app;
revoke insert,update,delete on public.product_bundles,public.product_bundle_items from crm_app;
grant select on public.exchange_rate_snapshots to crm_app;
...
create policy "next actions follow organization scope" on public.next_best_actions for select to crm_app
  using(exists(
...
begin
  ...
  if upper(decision)='ACCEPTED' then
    insert into public.crm_tasks(
      ...
      coalesce(organization.owner_id,app_auth.current_user_id()),least(action.valid_until,now()+interval '7 days'),app_auth.current_user_id()
    ) returning * into task;
  end if;
  update public.next_best_actions set
    status=upper(decision),decision_reason=nullif(trim(reason),''),
    decided_by=app_auth.current_user_id(),decided_at=now(),draft_task_id=task.id,updated_at=now()
  where id=action.id returning * into action;
  return action;
end;
$$;

grant select on public.next_best_actions to crm_app;
revoke all on function public.generate_next_best_actions(uuid),
  public.decide_next_best_action(uuid,text,text) from public,crm_system;
grant execute on function public.generate_next_best_actions(uuid),
  public.decide_next_best_action(uuid,text,text) to crm_app;
