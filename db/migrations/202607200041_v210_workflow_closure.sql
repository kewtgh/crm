...
begin
  ...
  insert into public.opportunities(
    ...
    trim(title_zh),trim(title_en),amount,upper(currency),lead.owner_id,app_auth.current_user_id(),
...
create policy "leaders read progression rules" on public.grade_progression_rules for select to crm_app
...
create policy "leaders manage progression rules" on public.grade_progression_rules for all to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'))
...
begin
  ...
  if target_rule is null then
    insert into public.grade_progression_rules(
      ...
      upper(trim(rule_action)),rule_active,app_auth.current_user_id()
    ) returning * into result;
  else
...
begin
  ...
  ) values(ws,target_household,target_contact,normalized_role,is_primary,app_auth.current_user_id())
...
begin
  ...
  insert into public.student_guardian_relationships(
    ...
    is_primary,is_emergency,has_legal_authority,app_auth.current_user_id()
  )
...
alter table public.ai_suggestions add column if not exists owner_id uuid references app_auth.accounts(id);
...
alter table public.ai_suggestions alter column owner_id set default app_auth.current_user_id();
...
create policy "members read suggestions" on public.ai_suggestions for select to crm_app
  using(public.is_workspace_member(workspace_id) and owner_id=app_auth.current_user_id());
...
begin
  ...
  where workspace_id=ws and owner_id=app_auth.current_user_id() and status='OPEN' and expires_at<=now();
  insert into public.ai_suggestion_runs(
    ...
    ws,'v2.1.0',
    encode(extensions.digest(ws::text||app_auth.current_user_id()::text||current_date::text,'sha256'),'hex'),
  ...
  select run.workspace_id,run.id,app_auth.current_user_id(),'CONTRACT',contract.id,
  ...
  where contract.workspace_id=run.workspace_id
    ...
    and not exists(
      ...
      where existing.workspace_id=run.workspace_id and existing.owner_id=app_auth.current_user_id()
...
begin
  ...
  from (select 1) seed
  left join public.user_preferences preference on preference.user_id=app_auth.current_user_id();
  ...
  select jsonb_build_object(
    'todayTasks',(select count(*) from public.crm_tasks where workspace_id=ws and owner_id=app_auth.current_user_id() and status<>'DONE' and (due_at at time zone tz)::date=local_today),
    'overdueTasks',(select count(*) from public.crm_tasks where workspace_id=ws and owner_id=app_auth.current_user_id() and status<>'DONE' and due_at<now()),
    'pendingApprovals',(select count(*) from public.approval_requests where workspace_id=ws and status='PENDING' and (elevated or requester_id=app_auth.current_user_id())),
    'renewalsDue',(select count(*) from public.contracts where workspace_id=ws and status in ('ACTIVE','RENEWAL_PREP','NEGOTIATING','RISK') and end_date between local_today and local_today+90 and (elevated or owner_id=app_auth.current_user_id())),
    'riskContracts',(select count(*) from public.contracts where workspace_id=ws and status='RISK' and (elevated or owner_id=app_auth.current_user_id())),
    'activeProducts',(select count(*) from public.products where workspace_id=ws and active),
    'unreadNotifications',(select count(*) from public.user_notifications where workspace_id=ws and user_id=app_auth.current_user_id() and read_at is null),
    ...
    'monthRevenueByCurrency',coalesce((select jsonb_object_agg(currency,total) from (
      ...
      where payment.workspace_id=ws and payment.status='CONFIRMED'
        and payment.paid_at>=month_start and (elevated or contract.owner_id=app_auth.current_user_id())
    ...
    'focusTasks',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      from public.crm_tasks where workspace_id=ws and owner_id=app_auth.current_user_id() and status<>'DONE'
...
grant select,insert,update on public.grade_progression_rules to crm_app;
