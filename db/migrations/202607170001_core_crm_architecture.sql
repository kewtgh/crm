...
as $$ select exists(select 1 from public.workspace_memberships m where m.workspace_id = target_workspace and m.user_id = app_auth.current_user_id() and m.status = 'ACTIVE'); $$;

create or replace function public.current_workspace_id()
returns uuid language sql stable security definer set search_path=public,app_auth,extensions
as $$ select workspace_id from public.workspace_memberships where user_id = app_auth.current_user_id() and status = 'ACTIVE' order by created_at limit 1; $$;
...
create trigger on_auth_user_created_crm_membership after insert or update of raw_app_meta_data on app_auth.accounts for each row execute procedure public.handle_new_crm_membership();

create table if not exists public.audit_events (
  id bigint generated always as identity primary key,
  workspace_id uuid not null references public.workspaces(id),
  actor_id uuid references app_auth.accounts(id),
...
create table if not exists public.organizations (
  ...
  owner_id uuid references app_auth.accounts(id),
...
create table if not exists public.sales_team_members (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null default public.current_workspace_id() references public.workspaces(id),
  auth_user_id uuid references app_auth.accounts(id) on delete set null,
  name_zh text not null,
  name_en text not null,
...
create table if not exists public.payments (
  ...
  paid_at timestamptz,
  reference text,
  verified_by uuid references app_auth.accounts(id),
  created_at timestamptz not null default now()
);
...
create table if not exists public.reminders (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id),
  recipient_id uuid not null references app_auth.accounts(id),
...
create table if not exists public.user_notifications (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id),
  user_id uuid not null references app_auth.accounts(id),
  kind text not null,
...
begin
  if app_auth.current_user_id() is null or not public.is_workspace_member(public.current_workspace_id()) then raise exception 'approval_not_authorized'; end if;
  ...
  values(public.current_workspace_id(),next_number,request_kind,object_type,object_id,app_auth.current_user_id(),trim(business_reason)) returning * into created;
  insert into public.approval_actions(approval_request_id,actor_id,action,comment) values(created.id,app_auth.current_user_id(),'SUBMITTED',trim(business_reason));
  return created;
end; $$;
...
begin
  ...
  if manager<>app_auth.current_user_id() and actor_role not in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR') then raise exception 'performance_manager_scope'; end if;
  ...
  for item in select * from jsonb_array_elements(coalesce(plan_allocations,'[]'::jsonb)) loop
    ...
    values(target.id,member.id,member.role,upper(item->>'attributionType'),(item->>'amount')::numeric,app_auth.current_user_id());
  end loop;
  return target;
end; $$;
...
begin
  ...
  if target.manager_id<>app_auth.current_user_id() and public.crm_role() not in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR') then raise exception 'performance_not_authorized'; end if;
...
begin
  ...
  values(public.current_workspace_id(),upper(product_code),trim(product_name_zh),trim(product_name_en),upper(product_billing),trim(product_duration_zh),trim(product_duration_en),app_auth.current_user_id()) returning * into created;
  insert into public.product_prices(product_id,currency,amount,effective_from,created_by) values(created.id,upper(price_currency),price_amount,current_date,app_auth.current_user_id());
  return created;
end; $$;
...
returns trigger language plpgsql security definer set search_path=public,app_auth,extensions as $$ begin perform public.generate_contract_reminders(new.id); return new; end; $$;
...
returns trigger language plpgsql security definer set search_path=public,app_auth,extensions as $$ begin perform public.generate_appointment_reminders(new.id); return new; end; $$;
...
begin
  ...
  values(ws,app_auth.current_user_id(),tg_table_name,entity,tg_op,before_row,after_row);
...
create policy "members read workspaces" on public.workspaces for select to crm_app using(public.is_workspace_member(id));
create policy "members read memberships" on public.workspace_memberships for select to crm_app using(user_id=app_auth.current_user_id() or (public.is_workspace_member(workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN')));
create policy "admins manage memberships" on public.workspace_memberships for all to crm_app using(public.is_workspace_member(workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN')) with check(public.is_workspace_member(workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN'));
create policy "admins read audit" on public.audit_events for select to crm_app using(public.is_workspace_member(workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN'));
...
do $$ declare table_name text; begin
  foreach table_name in array array['organizations','contacts','crm_tasks','sales_team_members','products','contracts','payments','appointments'] loop
    execute format('create policy "workspace members read %1$s" on public.%1$I for select to crm_app using(public.is_workspace_member(workspace_id))',table_name);
    execute format('create policy "workspace members insert %1$s" on public.%1$I for insert to crm_app with check(public.is_workspace_member(workspace_id))',table_name);
    execute format('create policy "workspace members update %1$s" on public.%1$I for update to crm_app using(public.is_workspace_member(workspace_id)) with check(public.is_workspace_member(workspace_id))',table_name);
    execute format('create policy "workspace admins delete %1$s" on public.%1$I for delete to crm_app using(public.is_workspace_member(workspace_id) and public.crm_role() in (''SUPER_ADMIN'',''ADMIN''))',table_name);
  end loop;
end $$;

create policy "members read product prices" on public.product_prices for select to crm_app using(exists(select 1 from public.products p where p.id=product_id and public.is_workspace_member(p.workspace_id)));
create policy "admins manage product prices" on public.product_prices for all to crm_app using(exists(select 1 from public.products p where p.id=product_id and public.is_workspace_member(p.workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'))) with check(exists(select 1 from public.products p where p.id=product_id and public.is_workspace_member(p.workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR')));
create policy "users read preferences" on public.user_preferences for select to crm_app using(user_id=app_auth.current_user_id());
create policy "users insert preferences" on public.user_preferences for insert to crm_app with check(user_id=app_auth.current_user_id() and public.is_workspace_member(workspace_id));
create policy "users update preferences" on public.user_preferences for update to crm_app using(user_id=app_auth.current_user_id()) with check(user_id=app_auth.current_user_id() and public.is_workspace_member(workspace_id));
create policy "users read reminders" on public.reminders for select to crm_app using(recipient_id=app_auth.current_user_id());
create policy "users manage reminders" on public.reminders for update to crm_app using(recipient_id=app_auth.current_user_id()) with check(recipient_id=app_auth.current_user_id());
create policy "users read notifications" on public.user_notifications for select to crm_app using(user_id=app_auth.current_user_id());
create policy "users update notifications" on public.user_notifications for update to crm_app using(user_id=app_auth.current_user_id()) with check(user_id=app_auth.current_user_id());
create policy "admins read outbox" on public.notification_outbox for select to crm_app using(public.is_workspace_member(workspace_id) and public.crm_role() in ('SUPER_ADMIN','ADMIN'));

drop policy if exists "approval participants can read" on public.approval_requests;
create policy "approval participants can read" on public.approval_requests for select to crm_app using(public.is_workspace_member(workspace_id) and (requester_id=app_auth.current_user_id() or public.crm_role() in ('ADMIN','SUPER_ADMIN')));
drop policy if exists "crm_app users submit approvals" on public.approval_requests;
create policy "crm_app users submit approvals" on public.approval_requests for insert to crm_app with check(public.is_workspace_member(workspace_id) and requester_id=app_auth.current_user_id() and status='PENDING' and decided_by is null);
drop policy if exists "sales hierarchy reads targets" on public.performance_targets;
create policy "sales hierarchy reads targets" on public.performance_targets for select to crm_app using(public.is_workspace_member(workspace_id) and (manager_id=app_auth.current_user_id() or public.crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR')));
drop policy if exists "allocation participants read" on public.performance_allocations;
create policy "allocation participants read" on public.performance_allocations for select to crm_app using(contributor_id=app_auth.current_user_id() or exists(select 1 from public.sales_team_members m where m.id=contributor_member_id and m.auth_user_id=app_auth.current_user_id()) or exists(select 1 from public.performance_targets t where t.id=target_id and t.manager_id=app_auth.current_user_id()) or public.crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));

grant select on public.workspaces,public.workspace_memberships,public.organizations,public.contacts,public.crm_tasks,public.sales_team_members,public.products,public.product_prices,public.contracts,public.payments,public.user_preferences,public.appointments,public.reminders,public.user_notifications,public.monthly_consumption to crm_app;
grant insert,update on public.organizations,public.contacts,public.crm_tasks,public.products,public.contracts,public.payments,public.user_preferences,public.appointments to crm_app;
grant delete on public.organizations,public.contacts,public.crm_tasks,public.products,public.contracts,public.payments,public.appointments to crm_app;
grant insert,update,delete on public.sales_team_members,public.product_prices to crm_app;
grant update on public.reminders,public.user_notifications to crm_app;
grant select on public.audit_events,public.notification_outbox to crm_app;
grant usage,select on sequence public.audit_events_id_seq to crm_app;
revoke all on function public.create_approval(text,text,text,text) from public;
grant execute on function public.create_approval(text,text,text,text) to crm_app;
...
grant execute on function public.save_performance_plan(uuid,uuid,date,date,text,numeric,jsonb),public.submit_performance_plan(uuid,text),public.create_product_with_price(text,text,text,text,text,text,text,numeric) to crm_app;
revoke all on function public.crm_duplicate_check(text,text,text,text,text) from public;
grant execute on function public.crm_duplicate_check(text,text,text,text,text) to crm_app;
revoke all on function public.process_due_reminders(integer) from public,crm_system,crm_app;
grant execute on function public.process_due_reminders(integer) to crm_system, crm_worker;
grant execute on function public.is_workspace_member(uuid),public.current_workspace_id() to crm_app;
