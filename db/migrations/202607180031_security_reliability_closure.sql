...
after insert on app_auth.accounts
...
create table if not exists public.staff_identity_repair_jobs(
  ...
  target_user_id uuid not null references app_auth.accounts(id) on delete cascade,
...
create policy "administrators read identity repair jobs"
  on public.staff_identity_repair_jobs for select to crm_app
  using(public.is_workspace_member(workspace_id)
    and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));
grant select on public.staff_identity_repair_jobs to crm_app;
revoke insert,update,delete on public.staff_identity_repair_jobs from crm_app;
...
begin
  ...
  insert into public.audit_events(
    ...
    result.workspace_id,app_auth.current_user_id(),'staff_identity_repair',result.id::text,
...
grant execute on function public.complete_identity_repair(uuid,boolean,text) to crm_system, crm_worker;
...
begin
  ...
  insert into public.audit_events(
    ...
    ws,app_auth.current_user_id(),'operational_job',job_id::text,'RETRY',
      ...
      if not found then
        ...
        if (case source_consent.status when 'REVOKED' then 3 when 'EXPIRED' then 2 else 1 end)
          ...
          update public.contact_consents set
            ...
            updated_by=app_auth.current_user_id(),updated_at=now()
