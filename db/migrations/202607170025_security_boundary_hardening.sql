...
revoke all on function public.refund_payment(uuid,text) from public,crm_system,crm_app;
...
revoke insert on public.approval_requests,public.approval_actions from crm_app;
...
begin
  ...
  if app_auth.current_user_id() is null or ws is null or actor_role='' or nullif(trim(business_reason),'') is null then
  ...
  if request_kind in ('CONTRACT_SIGN','CONTRACT_EXPORT') then
    ...
    if not found or target_row.status<>'PENDING_APPROVAL'
      or (actor_role='SALES_MANAGER' and target_row.manager_id<>app_auth.current_user_id()) then
    ...
    select * into refund_row from public.refunds
      where id=object_uuid and workspace_id=ws and status='PENDING_APPROVAL' and requested_by=app_auth.current_user_id();
  ...
  insert into public.approval_requests(
    ...
    ws,next_number,request_kind,object_type,object_id,app_auth.current_user_id(),trim(business_reason),now()+interval '7 days'
...
begin
  ...
  insert into public.quote_versions(
    ...
    trim(coalesce(terms_zh,'')),trim(coalesce(terms_en,'')),app_auth.current_user_id()
  );
  return result;
end;
$$;
...
begin
  ...
  insert into public.refunds(workspace_id,refund_number,payment_id,amount,reason,requested_by)
    values(
      ...
      payment.id,refund_amount,trim(refund_reason),app_auth.current_user_id()
    ) returning * into result;
  ...
  if jsonb_typeof(attendees)='array' then
    for attendee in select * from jsonb_array_elements(attendees) loop
      ...
      insert into public.appointment_attendees(
        ...
        lower(trim(attendee->>'email'))::citext,trim(coalesce(attendee->>'name','')),true,app_auth.current_user_id()
