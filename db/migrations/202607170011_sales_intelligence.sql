...
create table if not exists public.relationship_milestones (
  ...
  achieved_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
...
create table if not exists public.relationship_target_settings (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null default public.current_workspace_id() references public.workspaces(id),
  manager_id uuid references app_auth.accounts(id),
...
create policy "scoped read opportunities" on public.opportunities for select to crm_app
  using (public.can_access_owned_record(workspace_id,'OPPORTUNITY',id,owner_id,false));
create policy "staff create opportunities" on public.opportunities for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and owner_id=app_auth.current_user_id() and created_by=app_auth.current_user_id());
create policy "scoped update opportunities" on public.opportunities for update to crm_app
...
create policy "admins delete opportunities" on public.opportunities for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));

create policy "scoped activities" on public.crm_activities for select to crm_app
  using (public.is_workspace_member(workspace_id) and (owner_id=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR') or exists(
...
create policy "staff add activities" on public.crm_activities for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and owner_id=app_auth.current_user_id() and created_by=app_auth.current_user_id());

create policy "scoped relationship milestones" on public.relationship_milestones for select to crm_app
...
create policy "staff record relationship milestones" on public.relationship_milestones for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and achieved_by=app_auth.current_user_id() and exists(select 1 from public.organizations o where o.id=organization_id and public.can_access_owned_record(o.workspace_id,'ORGANIZATION',o.id,o.owner_id,true)));
create policy "staff update relationship milestones" on public.relationship_milestones for update to crm_app
...
create policy "relationship target readers" on public.relationship_target_settings for select to crm_app
  using (public.is_workspace_member(workspace_id));
create policy "relationship target managers" on public.relationship_target_settings for all to crm_app
...
create policy "scoped account plans" on public.account_plans for select to crm_app
  using (exists(select 1 from public.organizations o where o.id=organization_id and public.can_access_owned_record(o.workspace_id,'ORGANIZATION',o.id,o.owner_id,false)));
create policy "staff manage account plans" on public.account_plans for all to crm_app
  using (public.can_access_owned_record(workspace_id,'ACCOUNT_PLAN',id,owner_id,true))
  with check (public.is_workspace_member(workspace_id) and owner_id=app_auth.current_user_id());

create policy "contribution readers" on public.performance_contributions for select to crm_app
  using (public.is_workspace_member(workspace_id) and (public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER') or exists(select 1 from public.sales_team_members m where m.id=contributor_member_id and m.auth_user_id=app_auth.current_user_id())));
create policy "contract history readers" on public.contract_versions for select to crm_app
...
create policy "contract document readers" on public.contract_documents for select to crm_app
  using (exists(select 1 from public.contracts c where c.id=contract_id and public.can_access_owned_record(c.workspace_id,'CONTRACT',c.id,c.owner_id,false)));

grant select,insert,update,delete on public.opportunities to crm_app;
grant select,insert on public.crm_activities to crm_app;
grant select,insert,update on public.relationship_milestones to crm_app;
grant select,insert,update,delete on public.relationship_target_settings to crm_app;
grant select,insert,update,delete on public.account_plans to crm_app;
grant select on public.performance_contributions,public.contract_versions,public.contract_documents to crm_app;
...
returns public.relationship_milestones language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(organization.workspace_id,organization.id,milestone,trim(coalesce(evidence,'')),app_auth.current_user_id())
  on conflict(workspace_id,organization_id,milestone_type) do update set achieved_at=now(),evidence_note=excluded.evidence_note,evidence_status='RECORDED',achieved_by=app_auth.current_user_id(),verified_by=null,verified_at=null,updated_at=now()
  returning * into result;
  insert into public.audit_events(workspace_id,actor_id,action,entity_type,entity_id,after_data) values(organization.workspace_id,app_auth.current_user_id(),'RELATIONSHIP_MILESTONE_RECORDED','relationship_milestone',result.id,jsonb_build_object('milestoneType',milestone));
  return result;
end; $$;
...
returns public.relationship_target_settings language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  if actor_role='SALES_MANAGER' and target_manager is distinct from app_auth.current_user_id() then raise exception 'relationship_target_not_authorized'; end if;
  ...
  values(ws,target_manager,period_from,period_to,contact_percent,meal_percent,family_percent,advocacy_percent,app_auth.current_user_id())
...
begin
  ...
  update public.payments set status='CONFIRMED',paid_at=coalesce(paid_at,now()),reference=trim(payment_reference),verified_by=app_auth.current_user_id() where id=result.id returning * into result;
  for item in select * from jsonb_array_elements(contribution_allocations) loop
    ...
    values(result.workspace_id,result.id,(item->>'memberId')::uuid,item->>'type',(item->>'amount')::numeric,app_auth.current_user_id(),now(),app_auth.current_user_id());
  end loop;
  return result;
end; $$;
...
begin
  ...
  update public.payments set status='REFUNDED',verified_by=app_auth.current_user_id() where id=target_payment and workspace_id=public.current_workspace_id() and status='CONFIRMED' returning * into result;
  if not found then raise exception 'payment_not_confirmed'; end if;
  insert into public.audit_events(workspace_id,actor_id,action,entity_type,entity_id,after_data) values(result.workspace_id,app_auth.current_user_id(),'PAYMENT_REFUNDED','payments',result.id,jsonb_build_object('reason',trim(refund_reason)));
  return result;
end; $$;
...
grant execute on function public.upsert_relationship_milestone(uuid,text,text),public.save_relationship_targets(date,date,uuid,integer,integer,integer,integer),public.confirm_payment(uuid,text,jsonb),public.refund_payment(uuid,text),public.sales_performance_report(text,text) to crm_app;
