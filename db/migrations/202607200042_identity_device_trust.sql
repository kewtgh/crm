...
revoke all on public.trusted_login_devices from public,crm_system,crm_app;
grant select,insert,update,delete on public.trusted_login_devices to crm_system, crm_worker;
...
returns uuid language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
returns boolean language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
as $$
  ...
  where device.user_id=app_auth.current_user_id() and device.revoked_at is null and device.expires_at>now()
...
begin
  update public.trusted_login_devices set revoked_at=now()
  where id=target_device and user_id=app_auth.current_user_id() and revoked_at is null
  returning true into changed;
  if coalesce(changed,false) then
    insert into public.audit_events(
      workspace_id,actor_id,entity_type,entity_id,action
    ) values(ws,app_auth.current_user_id(),'trusted_login_device',target_device::text,'TRUSTED_DEVICE_REVOKED');
  end if;
...
begin
  update public.trusted_login_devices set revoked_at=now()
  where user_id=app_auth.current_user_id() and revoked_at is null
  ...
  if affected>0 then
    insert into public.audit_events(
      ...
      ws,app_auth.current_user_id(),'trusted_login_device',app_auth.current_user_id()::text,'OTHER_TRUSTED_DEVICES_REVOKED',
...
begin
  ...
  select jsonb_build_object(
    ...
    'privileged_mfa',count(*) filter(where m.role in ('SUPER_ADMIN','ADMIN') and exists(
      select 1 from app_auth.totp_factors f where f.user_id=m.user_id and f.status = 'VERIFIED'
    )),
