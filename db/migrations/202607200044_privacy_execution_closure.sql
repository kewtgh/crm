...
create table if not exists public.privacy_executions(
  ...
  executed_by uuid references app_auth.accounts(id),
  ...
  for select to crm_app using(
    public.is_workspace_member(workspace_id) and exists(
      ...
      where request.id=request_id and (
        request.created_by=app_auth.current_user_id()
...
grant select on public.privacy_executions,public.privacy_restrictions to crm_app;
revoke insert,update,delete on public.privacy_executions,public.privacy_restrictions from crm_app;
drop policy if exists "privacy leaders manage requests" on public.privacy_requests;
revoke update,delete on public.privacy_requests from crm_app;
grant select,insert on public.privacy_requests to crm_app;
...
begin
  if app_auth.current_db_role()<>'service_role' or exported_rows<1 or artifact_sha256!~'^[a-f0-9]{64}$' then
...
as $$
begin
  if app_auth.current_db_role()<>'service_role' then raise exception 'privacy_export_completion_invalid'; end if;
...
begin
  ...
  if request_row.request_type in ('EXPORT','DELETION') then
    ...
    if request_row.status='IN_PROGRESS' and normalized_status='WAITING_APPROVAL' then
      ...
      if request_row.assigned_to=app_auth.current_user_id() then raise exception 'privacy_second_reviewer_required'; end if;
      if request_row.execution_task_id is null then raise exception 'privacy_approval_task_missing'; end if;
      update public.crm_tasks set status='DONE',completed_at=now(),updated_at=now(),owner_id=app_auth.current_user_id()
  ...
  update public.privacy_requests set
    ...
    assigned_to=case when normalized_status='WAITING_APPROVAL' then app_auth.current_user_id() when normalized_status in ('FULFILLED','REJECTED','CANCELLED') then assigned_to else app_auth.current_user_id() end,
  ...
  if normalized_status='FULFILLED' then return public.start_privacy_execution(request_row.id,app_auth.current_user_id()); end if;
...
revoke all on function public.privacy_scope_snapshot(uuid,uuid),
  ...
  public.fail_privacy_export_execution(uuid,uuid,text) from public,crm_system,crm_app;
grant execute on function public.complete_privacy_export_execution(uuid,uuid,text,timestamptz,integer,text),
  public.fail_privacy_export_execution(uuid,uuid,text) to crm_system, crm_worker;
revoke all on function public.manage_privacy_request(uuid,text,text,text) from public,crm_system;
grant execute on function public.manage_privacy_request(uuid,text,text,text) to crm_app;
