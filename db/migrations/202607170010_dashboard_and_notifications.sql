...
begin
  ...
  select jsonb_build_object(
    'todayTasks',(select count(*) from public.crm_tasks where workspace_id=ws and owner_id=app_auth.current_user_id() and status<>'DONE' and due_at::date=current_date),
    ...
    'renewalsDue',(select count(*) from public.contracts where workspace_id=ws and status in ('ACTIVE','RENEWAL_PREP','NEGOTIATING','RISK') and end_date between current_date and current_date+90 and (elevated or owner_id=app_auth.current_user_id())),
    ...
    'monthRevenueByCurrency',coalesce((select jsonb_object_agg(currency,total) from (
      ...
      where p.workspace_id=ws and p.status='CONFIRMED' and p.paid_at>=date_trunc('month',current_date) and (elevated or c.owner_id=app_auth.current_user_id()) group by p.currency
    ) revenue),'{}'::jsonb),
...
grant execute on function public.dashboard_snapshot() to crm_app;
