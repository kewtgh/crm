...
alter table public.automation_rules
  ...
  add column if not exists updated_by uuid references app_auth.accounts(id);
...
returns trigger language plpgsql set search_path=public,app_auth,extensions as $$
begin
  ...
  new.updated_by:=app_auth.current_user_id();
  new.updated_at:=now();
  return new;
end;
$$;
...
returns jsonb language plpgsql stable security definer set search_path=public,app_auth,extensions as $$
...
begin
  ...
  actor:=coalesce(event_row.actor_id,app_auth.current_user_id());
...
create policy "portal staff read access consents" on public.portal_access_consents for select to crm_app
...
grant select on public.portal_access_consents to crm_app;
...
) returns public.portal_invitations language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
begin
  ...
  values(ws,target_household,guardian,normalized_email,target_digest,target_expires_at,app_auth.current_user_id()) returning * into result;
  return result;
end;
$$;
...
begin
  if app_auth.current_db_role()<>'service_role' or target_digest!~'^[a-f0-9]{64}$'
...
returns public.portal_update_requests language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
begin
  ...
  update public.portal_update_requests set status=normalized,decision_note=trim(decision),
    applied_changes=applied,decided_by=app_auth.current_user_id(),decided_at=now(),updated_at=now()
    where id=request_row.id returning * into request_row;
  return request_row;
end;
$$;

revoke update on public.portal_invitations,public.portal_update_requests from crm_app;
...
begin
  ...
  values(thread.workspace_id,thread.id,'OUTBOUND',left(trim(target_body),10000),'QUEUED',app_auth.current_user_id(),left(trim(target_idempotency_key),160))
...
begin
  ...
  if not found or nullif(trim(target_body),'') is null or nullif(trim(target_idempotency_key),'') is null then raise exception 'communication_message_invalid'; end if;
  insert into public.communication_messages(workspace_id,thread_id,direction,body,delivery_status,sent_by,idempotency_key)
  values(thread.workspace_id,thread.id,'INBOUND',left(trim(target_body),10000),'RECEIVED',app_auth.current_user_id(),left(trim(target_idempotency_key),160))
  on conflict(workspace_id,idempotency_key) do update set idempotency_key=excluded.idempotency_key
    where communication_messages.thread_id=excluded.thread_id and communication_messages.direction=excluded.direction
      and communication_messages.body=excluded.body returning * into result;
  if not found then raise exception 'communication_idempotency_conflict'; end if;
...
returns jsonb language sql stable security definer set search_path=public,app_auth,extensions as $$
  with matched as (
    ...
    where thread.workspace_id=public.current_workspace_id() and app_auth.current_user_id() is not null
...
create table if not exists public.data_quality_rule_configs(
  ...
  updated_by uuid references app_auth.accounts(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),
  unique(workspace_id,rule_key)
);
alter table public.data_quality_rule_configs enable row level security;
create policy "quality leaders read rule configs" on public.data_quality_rule_configs for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
grant select on public.data_quality_rule_configs to crm_app;
...
returns public.data_quality_rule_configs language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
begin
  ...
  update public.data_quality_rule_configs set enabled=next_enabled,severity=upper(next_severity),
    updated_by=app_auth.current_user_id(),updated_at=now() where workspace_id=public.current_workspace_id() and rule_key=normalized returning * into result;
...
returns public.data_quality_issues language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
create policy "operations leaders read connector receipts" on public.connector_reconciliation_receipts for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));
grant select on public.connector_reconciliation_receipts to crm_app;
...
) returns public.connector_reconciliation_receipts language plpgsql security definer set search_path=public,app_auth,extensions as $$
declare result public.connector_reconciliation_receipts;
begin
  if app_auth.current_db_role()<>'service_role' or upper(target_provider) not in ('PAYMENT','ACCOUNTING','E_SIGNATURE')
...
revoke all on function public.preview_automation_rule(uuid,jsonb),public.retry_automation_run(uuid),
  ...
  public.growth_performance_snapshot() from public,crm_system;
grant execute on function public.preview_automation_rule(uuid,jsonb),public.retry_automation_run(uuid),
  public.create_guardian_portal_invitation(uuid,text,text,timestamptz),public.revoke_guardian_portal_invitation(uuid),
  public.decide_portal_update(uuid,text,text),public.queue_communication_message(uuid,text,text),
  public.record_inbound_communication(uuid,text,text),public.retry_communication_message(uuid),public.communication_inbox_snapshot(text,integer),
  public.configure_data_quality_rule(text,boolean,text),public.assign_data_quality_issue(uuid,uuid),
  public.growth_performance_snapshot() to crm_app;
