...
revoke all on function public.crm_resource_metrics(text,text,text) from public,crm_system;
grant execute on function public.crm_resource_metrics(text,text,text) to crm_app, crm_system, crm_worker;
...
begin
  ...
  from (select 1) seed
  left join public.user_preferences p on p.user_id=app_auth.current_user_id();
  ...
  select jsonb_build_object(
    ...
    'monthRevenueByCurrency',coalesce((select jsonb_object_agg(currency,total) from (
      ...
      where p.workspace_id=ws and p.status='CONFIRMED' and p.paid_at>=month_start and (elevated or c.owner_id=app_auth.current_user_id()) group by p.currency
    ) revenue),'{}'::jsonb),
    'focusTasks',coalesce((select jsonb_agg(jsonb_build_object('id',id,'titleZh',title_zh,'titleEn',title_en,'related',related_label,'status',status,'priority',priority,'dueAt',due_at) order by due_at nulls last) from (
      select id,title_zh,title_en,related_label,status,priority,due_at from public.crm_tasks where workspace_id=ws and owner_id=app_auth.current_user_id() and status<>'DONE' order by due_at nulls last limit 6
    ) tasks),'[]'::jsonb)
  ) into result;
  return result;
end;
$$;
revoke all on function public.dashboard_snapshot(text) from public,crm_system;
grant execute on function public.dashboard_snapshot(text) to crm_app, crm_system, crm_worker;
...
begin
  ...
  insert into public.next_action_generation_batches(
    ...
    public.current_workspace_id(),app_auth.current_user_id(),target_organization,generated
...
$$;
revoke all on function public.generate_next_best_actions(uuid) from public,crm_system;
grant execute on function public.generate_next_best_actions(uuid) to crm_app, crm_system, crm_worker;

-- Smoke tests and operational recovery must be able to snapshot and restore a
-- worker heartbeat without broadening access for crm_app CRM users.
grant select,insert,update,delete on public.worker_heartbeats to crm_system, crm_worker;
