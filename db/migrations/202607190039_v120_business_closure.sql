...
declare
  ...
  target_team text;
begin
  if app_auth.current_user_id() is null or target_owner is null then return false; end if;
  if target_owner=app_auth.current_user_id() then return true; end if;
  if not exists(
  ...
  select team into actor_team from public.sales_team_members
    where workspace_id=public.current_workspace_id() and auth_user_id=app_auth.current_user_id() and active limit 1;
...
as $$
  ...
  order by case when m.user_id=app_auth.current_user_id() then 0 else 1 end,p.display_name_en,m.user_id
  limit 30;
$$;
...
declare
  ...
  next_owner uuid;
begin
  if app_auth.current_user_id() is null or ws is null or patch is null or jsonb_typeof(patch)<>'object' then
...
begin
  ...
  if new.owner_id is not null and (tg_op='INSERT' or new.owner_id is distinct from old.owner_id)
    and new.owner_id<>coalesce(app_auth.current_user_id(),new.created_by) then
...
declare
  ...
  result jsonb;
begin
  if app_auth.current_user_id() is null then raise exception 'task_workspace_forbidden'; end if;
    ...
    'items',coalesce((
      ...
      from (
        ...
        where t.workspace_id=public.current_workspace_id()
          ...
          and (t.owner_id=app_auth.current_user_id() or (can_view_team and public.can_assign_crm_task(t.owner_id)))
...
begin
  if app_auth.current_user_id() is null or cardinality(task_ids) not between 1 and 50
  ...
  insert into public.audit_events(
    ...
    public.current_workspace_id(),app_auth.current_user_id(),'crm_task_bulk',gen_random_uuid()::text,
...
create policy "members read visible shared views" on public.shared_views for select to crm_app
  using(public.is_workspace_member(workspace_id) and (owner_id=app_auth.current_user_id() or visibility='TEAM'));
create policy "members create owned shared views" on public.shared_views for insert to crm_app
  with check(public.is_workspace_member(workspace_id) and owner_id=app_auth.current_user_id());
create policy "owners update shared views" on public.shared_views for update to crm_app
  using(owner_id=app_auth.current_user_id()) with check(owner_id=app_auth.current_user_id() and public.is_workspace_member(workspace_id));
create policy "owners delete shared views" on public.shared_views for delete to crm_app
  using(owner_id=app_auth.current_user_id());
...
begin
  ...
  insert into public.shared_views(
    ...
    public.current_workspace_id(),p_resource_key,trim(p_view_name),p_view_visibility,p_view_config,app_auth.current_user_id()
...
begin
  ...
  where id=target_view and workspace_id=public.current_workspace_id() and owner_id=app_auth.current_user_id();
...
as $$
  ...
  where workspace_id=public.current_workspace_id()
    ...
    and operation=p_operation_name and created_by=app_auth.current_user_id();
$$;
...
begin
  ...
  insert into public.mutation_receipts(
    ...
    public.current_workspace_id(),p_request_key,p_operation_name,coalesce(p_operation_result,'{}'::jsonb),app_auth.current_user_id()
...
begin
  ...
  values(public.current_workspace_id(),p_request_key,'DUPLICATE_MERGE',jsonb_build_object('id',merged_id),app_auth.current_user_id());
  return merged_id;
end;
$$;
...
begin
  if app_auth.current_user_id() is null or length(p_request_key) not between 8 and 160 then
    raise exception 'mutation_receipt_invalid';
  end if;
  perform pg_advisory_xact_lock(hashtextextended(public.current_workspace_id()::text||':'||p_request_key,0));
  ...
  insert into public.mutation_receipts(workspace_id,request_key,operation,result,created_by)
  values(public.current_workspace_id(),p_request_key,'IMPORT_ROLLBACK',to_jsonb(result),app_auth.current_user_id());
  return result;
end;
$$;
...
begin
  ...
  insert into public.mutation_receipts(workspace_id,request_key,operation,result,created_by)
  values(public.current_workspace_id(),p_request_key,'QUOTE_ACCEPT',to_jsonb(result),app_auth.current_user_id());
  return result;
end;
$$;
...
begin
  ...
  insert into public.mutation_receipts(workspace_id,request_key,operation,result,created_by)
  values(public.current_workspace_id(),p_request_key,'PRODUCT_ACTIVE',to_jsonb(result),app_auth.current_user_id());
  return result;
end;
$$;
...
begin
  ...
  if dismiss then
    update public.data_quality_issues set status='DISMISSED',
      resolution_note=trim(resolution),resolved_by=app_auth.current_user_id(),resolved_at=now()
  ...
  update public.data_quality_issues
    set resolution_note=trim(resolution),resolved_by=app_auth.current_user_id(),resolved_at=now()
    where id=target_issue returning * into result;
  return result;
end;
$$;
...
begin
  resource_key:=lower(trim(resource_key));
  if app_auth.current_user_id() is null or resource_key not in ('schools','people','tasks')
  ...
  insert into public.approval_requests(
    ...
    jsonb_build_object(
      ...
      'scope',scope_value,'requesterId',app_auth.current_user_id()
    )
...
begin
  ...
  update public.approval_requests set
    ...
    decided_by=app_auth.current_user_id(),decided_at=now(),updated_at=now()
  where id=request_id returning * into request;
  insert into public.approval_actions(approval_request_id,actor_id,action,comment)
    values(request_id,app_auth.current_user_id(),decision,nullif(trim(decision_comment),''));
  begin
...
grant execute on function
  ...
  public.idempotent_set_product_active(uuid,boolean,text)
to crm_app;
grant execute on function public.apply_account_recovery_throttle(text,text) to crm_system, crm_worker;
grant select,insert,update,delete on public.shared_views to crm_app;
grant select on public.recovery_throttle_buckets to crm_system, crm_worker;
