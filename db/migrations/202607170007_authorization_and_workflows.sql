...
as $$
  select coalesce((
    ...
    where user_id = app_auth.current_user_id() and status = 'ACTIVE'
    order by created_at
    limit 1
  ), '');
$$;
...
grant execute on function public.current_crm_role() to crm_app;
...
create table if not exists public.record_collaborators (
  ...
  granted_by uuid not null references app_auth.accounts(id),
...
create policy "collaborators read own grants" on public.record_collaborators for select to crm_app
  using (user_id = app_auth.current_user_id() or (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR')));
create policy "leaders manage collaborator grants" on public.record_collaborators for all to crm_app
  ...
  with check (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER') and granted_by = app_auth.current_user_id());
...
grant execute on function public.can_access_owned_record(uuid,text,uuid,uuid,boolean) to crm_app;
...
create policy "scoped read organizations" on public.organizations for select to crm_app
  using (public.can_access_owned_record(workspace_id,'ORGANIZATION',id,owner_id,false));
create policy "sales create owned organizations" on public.organizations for insert to crm_app
...
create policy "scoped update organizations" on public.organizations for update to crm_app
...
create policy "admins delete organizations" on public.organizations for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

create policy "scoped read contacts" on public.contacts for select to crm_app
  using (public.can_access_owned_record(workspace_id,'CONTACT',id,owner_id,false));
create policy "sales create owned contacts" on public.contacts for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and owner_id=app_auth.current_user_id() and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST'));
create policy "scoped update contacts" on public.contacts for update to crm_app
...
create policy "admins delete contacts" on public.contacts for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

create policy "scoped read tasks" on public.crm_tasks for select to crm_app
  using (public.can_access_owned_record(workspace_id,'TASK',id,owner_id,false));
create policy "members create owned tasks" on public.crm_tasks for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and owner_id=app_auth.current_user_id());
create policy "scoped update tasks" on public.crm_tasks for update to crm_app
...
create policy "admins delete tasks" on public.crm_tasks for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

create policy "members read sales directory" on public.sales_team_members for select to crm_app
  using (public.is_workspace_member(workspace_id));
create policy "leaders insert sales members" on public.sales_team_members for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));
create policy "leaders update sales members" on public.sales_team_members for update to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'))
  with check (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));
create policy "admins delete sales members" on public.sales_team_members for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

create policy "members read products" on public.products for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "catalog leaders create products" on public.products for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));
create policy "catalog leaders update products" on public.products for update to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'))
  with check (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'));
create policy "admins delete products" on public.products for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

create policy "scoped read contracts" on public.contracts for select to crm_app
  using (public.can_access_owned_record(workspace_id,'CONTRACT',id,owner_id,false));
create policy "sales create owned contracts" on public.contracts for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and owner_id=app_auth.current_user_id() and status='DRAFT' and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST'));
create policy "sales update draft contracts" on public.contracts for update to crm_app
...
create policy "admins delete contracts" on public.contracts for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

create policy "finance scoped read payments" on public.payments for select to crm_app
...
create policy "scoped read appointments" on public.appointments for select to crm_app
  using (public.can_access_owned_record(workspace_id,'APPOINTMENT',id,owner_id,false));
create policy "members create owned appointments" on public.appointments for insert to crm_app
  with check (public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and owner_id=app_auth.current_user_id());
create policy "scoped update appointments" on public.appointments for update to crm_app
...
create policy "admins delete appointments" on public.appointments for delete to crm_app
  using (public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));

revoke insert, update, delete on public.payments from crm_app;
...
create table if not exists public.generated_jobs (
  ...
  created_by uuid not null references app_auth.accounts(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
alter table public.generated_jobs enable row level security;
create policy "job participants read" on public.generated_jobs for select to crm_app
  using (public.is_workspace_member(workspace_id) and (created_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN')));
...
begin
  if app_auth.current_user_id() is null or ws is null then raise exception 'approval_not_authorized'; end if;
  ...
  values(ws,next_number,request_kind,object_type,object_id,app_auth.current_user_id(),trim(business_reason),now()+interval '7 days') returning * into created;
...
begin
  if app_auth.current_user_id() is null or actor_role not in ('ADMIN','SUPER_ADMIN') then raise exception 'approval_not_authorized'; end if;
  ...
  if request.requester_id=app_auth.current_user_id() then raise exception 'approval_self_decision_forbidden'; end if;
  ...
  update public.approval_requests set status=decision,decision_reason=nullif(trim(decision_comment),''),decided_by=app_auth.current_user_id(),decided_at=now(),updated_at=now() where id=request_id returning * into request;
...
begin
  select * into request from public.approval_requests where id=request_id and workspace_id=public.current_workspace_id() for update;
  if not found or request.status<>'PENDING' or request.requester_id<>app_auth.current_user_id() then raise exception 'approval_withdraw_forbidden'; end if;
  ...
  insert into public.approval_actions(approval_request_id,actor_id,action,comment) values(request_id,app_auth.current_user_id(),'CANCELLED',trim(withdrawal_reason));
...
grant execute on function public.create_approval(text,text,text,text), public.decide_approval(uuid,text,text), public.withdraw_approval(uuid,text) to crm_app;
...
create policy "target owners update drafts" on public.performance_targets for update to crm_app
  using ((manager_id=app_auth.current_user_id() and status='DRAFT') or public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR'))
...
grant select on public.record_collaborators, public.generated_jobs to crm_app;
grant insert,update,delete on public.record_collaborators to crm_app;
...
as $$
  with staff as (
    select u.id user_id, p.username::text, p.display_name_zh, p.display_name_en, u.email::text,
      ...
      exists(select 1 from app_auth.totp_factors f where f.user_id=u.id and f.status = 'VERIFIED') mfa_enabled
    from app_auth.accounts u
...
grant execute on function public.list_staff_users(text,integer,integer) to crm_app;

revoke insert,update,delete on public.performance_targets,public.performance_allocations from crm_app;
