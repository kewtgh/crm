...
from app_auth.accounts u cross join lateral (select coalesce(nullif(lower(raw_user_meta_data->>'username'), ''), regexp_replace(lower(split_part(email, '@', 1)), '[^a-z0-9._-]', '', 'g')) as candidate) names where not exists (select 1 from public.user_profiles p where p.user_id = u.id);

alter table public.user_profiles enable row level security;
create policy "users read own profile" on public.user_profiles for select to crm_app using (app_auth.current_user_id() = user_id);
create policy "users update own names" on public.user_profiles for update to crm_app using (app_auth.current_user_id() = user_id) with check (app_auth.current_user_id() = user_id);
...
grant execute on function public.username_available(text) to crm_system, crm_app;
...
create trigger on_auth_user_created_profile after insert on app_auth.accounts for each row execute procedure public.handle_new_lumina_crm_user();
