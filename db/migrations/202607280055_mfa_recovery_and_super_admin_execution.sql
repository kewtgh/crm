...
create policy mfa_recovery_codes_owner_select on public.mfa_recovery_codes for select to crm_app using (user_id = app_auth.current_user_id());
create policy mfa_recovery_codes_owner_insert on public.mfa_recovery_codes for insert to crm_app with check (user_id = app_auth.current_user_id());
create policy mfa_recovery_codes_owner_delete on public.mfa_recovery_codes for delete to crm_app using (user_id = app_auth.current_user_id());
revoke all on public.mfa_recovery_codes from public;
grant select, insert, delete on public.mfa_recovery_codes to crm_app;
...
begin
  if app_auth.current_user_id() is null or public.current_crm_role() <> 'SUPER_ADMIN' then
  ...
  insert into public.approval_actions(approval_request_id,actor_id,action,comment)
    values(request_id,app_auth.current_user_id(),'APPROVED','SUPER_ADMIN_DIRECT_EXECUTION');
...
grant execute on function public.super_admin_execute_approval(uuid) to crm_app;
