...
grant execute on function public.contract_summary() to crm_app;
...
grant execute on function public.consumption_report(text,text) to crm_app;
...
returns setof public.notification_outbox language plpgsql security definer set search_path=public,app_auth,extensions
as $$ begin
...
returns void language sql security definer set search_path=public,app_auth,extensions as $$ update public.notification_outbox set status='SENT',delivered_at=now(),last_error=null,updated_at=now() where id=job_id; $$;
create or replace function public.fail_notification_outbox(job_id uuid, failure text)
returns void language sql security definer set search_path=public,app_auth,extensions as $$ update public.notification_outbox set status=case when attempts>=8 then 'DEAD' else 'FAILED' end,last_error=left(failure,500),next_attempt_at=now()+make_interval(mins=>least(360,power(2,attempts)::int)),updated_at=now() where id=job_id; $$;
revoke all on function public.claim_notification_outbox(integer),public.complete_notification_outbox(uuid),public.fail_notification_outbox(uuid,text) from public,crm_system,crm_app;
grant execute on function public.claim_notification_outbox(integer),public.complete_notification_outbox(uuid),public.fail_notification_outbox(uuid,text) to crm_system, crm_worker;
