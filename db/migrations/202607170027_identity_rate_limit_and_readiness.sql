...
grant execute on function public.apply_login_throttle(text,text,text) to crm_system, crm_worker;
...
create table if not exists public.staff_identity_changes(
  ...
  actor_user_id uuid not null references app_auth.accounts(id),
...
create policy "administrators read identity changes"
  on public.staff_identity_changes for select to crm_app
...
revoke insert,update,delete on public.workspace_memberships,public.sales_team_members from crm_app;
...
grant select on public.staff_identity_changes to crm_app;
