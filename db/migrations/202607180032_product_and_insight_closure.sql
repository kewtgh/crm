...
begin
  ...
  insert into public.contract_renewal_playbooks(
    ...
    trim(action_zh),trim(action_en),action_due,coalesce(contract.owner_id,app_auth.current_user_id()),
    trim(coalesce(outcome,'')),(context->>'healthScore')::integer,
    (context->'suggestion'->>'windowDays')::integer,now(),app_auth.current_user_id(),app_auth.current_user_id()
    ...
    last_auto_evaluated_at=now(),updated_by=app_auth.current_user_id(),updated_at=now()
...
create policy "administrators read integration sync jobs"
  on public.integration_sync_jobs for select to crm_app
  using(public.is_workspace_member(workspace_id)
    and public.current_crm_role() in ('SUPER_ADMIN','ADMIN'));
grant select on public.integration_sync_jobs to crm_app;
...
begin
  ...
  update public.integration_connections set
    ...
    external_account_label=trim(coalesce(account_label,'')),configured_by=app_auth.current_user_id(),
...
begin
  ...
  insert into public.integration_sync_jobs(
    ...
    connection.sync_direction,connection.cursor_value,app_auth.current_user_id()
  ) returning * into result;
  return result;
end;
$$;
...
grant execute on function public.configure_integration(text,text,text,text),
  public.request_integration_sync(text) to crm_app;
...
grant execute on function public.service_readiness_snapshot(uuid) to crm_system, crm_worker;
...
create table if not exists public.next_action_generation_batches(
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  requested_by uuid references app_auth.accounts(id),
...
create policy "members read NBA batches" on public.next_action_generation_batches
  for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "members read NBA evaluations" on public.next_action_evaluations
  for select to crm_app using(public.is_workspace_member(workspace_id));
grant select on public.next_action_generation_batches,public.next_action_evaluations to crm_app;
...
revoke all on function public.generate_next_best_actions(uuid),
  public.business_improvement_snapshot() from public,crm_system;
grant execute on function public.generate_next_best_actions(uuid),
  public.business_improvement_snapshot() to crm_app;
