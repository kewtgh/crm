...
begin
  select * into batch from public.import_batches
    where id=target_batch and workspace_id=public.current_workspace_id()
      and created_by=app_auth.current_user_id() for update;
    ...
    begin
      ...
      if coalesce(item.decision,'CREATE')='CREATE' and batch.resource_type='CONTACTS' then
        insert into public.contacts(
          ...
          'UNVERIFIED',app_auth.current_user_id(),app_auth.current_user_id()
        ...
        insert into public.organizations(
          ...
          batch.workspace_id,item.normalized_data->>'nameZh',item.normalized_data->>'nameEn',
          item.normalized_data->>'city','UNVERIFIED',app_auth.current_user_id(),app_auth.current_user_id()
        ) returning id into entity_id;
...
begin
  select * into batch from public.import_batches
    where id=target_batch and workspace_id=public.current_workspace_id()
      and created_by=app_auth.current_user_id() and status in ('COMPLETED','PARTIAL_FAILED') for update;
