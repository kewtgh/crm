...
begin
  ...
  if target.manager_id<>app_auth.current_user_id() and public.current_crm_role() not in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR') then raise exception 'performance_not_authorized'; end if;
...
begin
  if tg_op='INSERT' or (to_jsonb(new)-'updated_at') is distinct from (to_jsonb(old)-'updated_at') then
    ...
    values(new.workspace_id,new.id,next_version,to_jsonb(new)-'created_by',case when tg_op='INSERT' then 'CONTRACT_CREATED' else 'CONTRACT_UPDATED' end,coalesce(app_auth.current_user_id(),new.created_by));
  end if;
  return new;
end; $$;
...
returns public.product_prices language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  insert into public.product_prices(product_id,currency,amount,effective_from,created_by) values(target_product,price_currency,price_amount,effective_on,app_auth.current_user_id()) returning * into result;
  return result;
end; $$;

revoke all on function public.set_product_price(uuid,text,numeric,date) from public;
grant execute on function public.set_product_price(uuid,text,numeric,date) to crm_app;
