...
create table if not exists public.contact_consents (
  ...
  updated_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
...
create table if not exists public.quotes (
  ...
  owner_id uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
  created_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.refunds (
  ...
  requested_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
  approved_by uuid references app_auth.accounts(id),
...
create table if not exists public.reconciliation_items (
  ...
  assigned_to uuid references app_auth.accounts(id),
  resolved_by uuid references app_auth.accounts(id),
...
create table if not exists public.import_rows (
  ...
  last_error text,
  decided_by uuid references app_auth.accounts(id),
...
returns boolean language sql stable security definer set search_path=public,app_auth,extensions
as $$
  select exists(
...
returns public.contact_consents language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(contact.workspace_id,contact.id,upper(target_channel),upper(target_purpose),upper(target_status),trim(consent_source),trim(coalesce(evidence,'')),case when upper(target_status)='GRANTED' then now() end,case when upper(target_status)='REVOKED' then now() end,retained_until,quiet_start,quiet_end,app_auth.current_user_id(),app_auth.current_user_id())
  on conflict(workspace_id,contact_id,channel,purpose) do update set status=excluded.status,source=excluded.source,evidence_note=excluded.evidence_note,obtained_at=case when excluded.status='GRANTED' then now() else contact_consents.obtained_at end,revoked_at=case when excluded.status='REVOKED' then now() else null end,retention_until=excluded.retention_until,quiet_hours_start=excluded.quiet_hours_start,quiet_hours_end=excluded.quiet_hours_end,updated_by=app_auth.current_user_id(),updated_at=now()
...
returns public.quotes language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(organization.workspace_id,trim(quote_no),organization.id,target_opportunity,target_product,upper(quote_currency),valid_through,app_auth.current_user_id(),app_auth.current_user_id()) returning * into result;
  insert into public.quote_versions(workspace_id,quote_id,version,subtotal,discount_amount,terms_zh,terms_en,created_by)
  values(result.workspace_id,result.id,1,quote_subtotal,quote_discount,trim(coalesce(terms_zh,'')),trim(coalesce(terms_en,'')),app_auth.current_user_id());
  return result;
end; $$;
...
returns public.quote_versions language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(quote.workspace_id,quote.id,next_version,quote_subtotal,quote_discount,trim(coalesce(terms_zh,'')),trim(coalesce(terms_en,'')),trim(change_reason),app_auth.current_user_id()) returning * into result;
...
begin
  ...
  values(quote.workspace_id,trim(contract_no),quote.organization_id,quote.product_id,period_start,period_end,quote.currency,version.total_amount,'DRAFT',quote.owner_id,app_auth.current_user_id(),quote.id) returning * into result;
...
returns setof public.receivable_schedules language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  for item in select * from jsonb_array_elements(installments) loop
    index_no:=index_no+1;
    insert into public.receivable_schedules(workspace_id,contract_id,installment_number,due_date,amount,created_by) values(contract.workspace_id,contract.id,index_no,(item->>'dueDate')::date,(item->>'amount')::numeric,app_auth.current_user_id());
  end loop;
...
returns public.payments language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(contract.workspace_id,contract.id,contract.product_id,schedule.id,payment_amount,contract.currency,'CONFIRMED',coalesce(paid_on,now()),trim(payment_reference),app_auth.current_user_id(),'MATCHED') returning * into result;
  ...
  values(contract.workspace_id,contract.id,result.id,payment_amount,payment_amount,'MATCHED',app_auth.current_user_id());
  return result;
end; $$;

create or replace function public.request_refund(target_payment uuid, refund_amount numeric, refund_reason text)
returns public.refunds language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(payment.workspace_id,'RF-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS'),payment.id,refund_amount,trim(refund_reason),app_auth.current_user_id()) returning * into result;
...
returns jsonb language plpgsql stable security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(public.current_workspace_id(),upper(resource),left(filename,180),content_hash,request_key,mapping,app_auth.current_user_id()) on conflict(workspace_id,idempotency_key) do update set idempotency_key=excluded.idempotency_key returning * into batch;
...
begin
  select b.* into batch from public.import_rows r join public.import_batches b on b.id=r.batch_id where r.id=target_row and b.workspace_id=public.current_workspace_id() and b.created_by=app_auth.current_user_id();
  if not found or upper(chosen_action) not in ('CREATE','UPDATE','MERGE','SKIP') then raise exception 'import_decision_invalid'; end if;
  update public.import_rows set decision=upper(chosen_action),status=case when upper(chosen_action)='SKIP' then 'SKIPPED' else 'DECIDED' end,decided_by=app_auth.current_user_id(),decided_at=now() where id=target_row and status='DUPLICATE' returning * into result;
  ...
  for item in select * from public.import_rows where batch_id=batch.id and status in ('VALID','DECIDED') order by row_number for update skip locked limit greatest(1,least(batch_size,100)) loop
    begin
      ...
      if coalesce(item.decision,'CREATE')='CREATE' and batch.resource_type='CONTACTS' then
        insert into public.contacts(workspace_id,name_zh,name_en,email,phone,title,status,owner_id,created_by) values(batch.workspace_id,item.normalized_data->>'nameZh',item.normalized_data->>'nameEn',nullif(item.normalized_data->>'email','')::citext,nullif(item.normalized_data->>'phone',''),item.normalized_data->>'title','UNVERIFIED',app_auth.current_user_id(),app_auth.current_user_id()) returning id into entity_id;
      elsif coalesce(item.decision,'CREATE')='CREATE' then
        insert into public.organizations(workspace_id,name_zh,name_en,city,status,owner_id,created_by) values(batch.workspace_id,item.normalized_data->>'nameZh',item.normalized_data->>'nameEn',item.normalized_data->>'city','UNVERIFIED',app_auth.current_user_id(),app_auth.current_user_id()) returning id into entity_id;
...
returns integer language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  on conflict(workspace_id,rule_key,entity_type,entity_id) do update set status=case when data_quality_issues.status='RESOLVED' then 'OPEN' else data_quality_issues.status end,details=excluded.details,last_seen_at=marker;
  update public.data_quality_issues set status='RESOLVED',resolution_note='AUTO_RESOLVED',resolved_at=now(),resolved_by=app_auth.current_user_id() where workspace_id=public.current_workspace_id() and status in ('OPEN','ASSIGNED') and last_seen_at<marker;
...
returns public.appointments language plpgsql security definer set search_path=public,app_auth,extensions
as $$
...
begin
  ...
  values(public.current_workspace_id(),trim(title_zh),trim(title_en),event_type,relation_type,relation_id,coalesce(relation_label,''),starts,ends,coalesce(event_channel,''),reminders,app_auth.current_user_id(),app_auth.current_user_id(),1) returning * into result;
  if jsonb_typeof(attendees)='array' then
    for attendee in select * from jsonb_array_elements(attendees) loop
      ...
      values(result.workspace_id,result.id,nullif(attendee->>'contactId','')::uuid,lower(trim(attendee->>'email'))::citext,trim(coalesce(attendee->>'name','')),true,app_auth.current_user_id()) returning * into attendee_row;
...
returns setof public.calendar_deliveries language plpgsql security definer set search_path=public,app_auth,extensions
as $$
begin
...
returns void language sql security definer set search_path=public,app_auth,extensions as $$ update public.calendar_deliveries set status='DELIVERED',delivered_at=now(),provider_message_id=provider_id,last_error=null,updated_at=now() where id=delivery_id and status='SENDING'; $$;

create or replace function public.fail_calendar_delivery(delivery_id uuid, failure text)
returns void language sql security definer set search_path=public,app_auth,extensions as $$ update public.calendar_deliveries set status='FAILED',last_error=left(failure,500),available_at=now()+make_interval(mins=>least(60,power(2,greatest(attempts,1))::integer)),updated_at=now() where id=delivery_id and status='SENDING'; $$;
...
create policy "scoped contact consents" on public.contact_consents for select to crm_app using(exists(select 1 from public.contacts c where c.id=contact_id and public.can_access_owned_record(c.workspace_id,'CONTACT',c.id,c.owner_id,false)));
create policy "scoped quotes" on public.quotes for select to crm_app using(public.can_access_owned_record(workspace_id,'QUOTE',id,owner_id,false));
create policy "scoped quote versions" on public.quote_versions for select to crm_app using(exists(select 1 from public.quotes q where q.id=quote_id and public.can_access_owned_record(q.workspace_id,'QUOTE',q.id,q.owner_id,false)));
create policy "scoped receivables" on public.receivable_schedules for select to crm_app using(exists(select 1 from public.contracts c where c.id=contract_id and public.can_access_owned_record(c.workspace_id,'CONTRACT',c.id,c.owner_id,false)));
create policy "refund participants" on public.refunds for select to crm_app using(public.is_workspace_member(workspace_id) and (requested_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN')));
create policy "reconciliation leaders" on public.reconciliation_items for select to crm_app using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
create policy "import owners read batches" on public.import_batches for select to crm_app using(public.is_workspace_member(workspace_id) and (created_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN')));
create policy "import owners read rows" on public.import_rows for select to crm_app using(exists(select 1 from public.import_batches b where b.id=batch_id and (b.created_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN'))));
create policy "quality leaders" on public.data_quality_issues for select to crm_app using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
create policy "appointment attendees scoped" on public.appointment_attendees for select to crm_app using(exists(select 1 from public.appointments a where a.id=appointment_id and public.can_access_owned_record(a.workspace_id,'APPOINTMENT',a.id,a.owner_id,false)));
create policy "calendar deliveries scoped" on public.calendar_deliveries for select to crm_app using(exists(select 1 from public.appointments a where a.id=appointment_id and public.can_access_owned_record(a.workspace_id,'APPOINTMENT',a.id,a.owner_id,false)));

grant select on public.contact_consents,public.quotes,public.quote_versions,public.receivable_schedules,public.refunds,public.reconciliation_items,public.import_batches,public.import_rows,public.data_quality_issues,public.appointment_attendees,public.calendar_deliveries to crm_app;
...
grant execute on function public.contact_channel_allowed(uuid,text,text),public.save_contact_consent(uuid,text,text,text,text,text,date,time,time),public.create_quote(text,uuid,uuid,uuid,text,numeric,numeric,date,text,text),public.add_quote_version(uuid,numeric,numeric,text,text,text),public.submit_quote(uuid,text),public.accept_quote(uuid),public.convert_quote_to_contract(uuid,text,date,date),public.save_receivable_schedule(uuid,jsonb),public.record_payment(uuid,uuid,numeric,text,text,timestamptz),public.request_refund(uuid,numeric,text),public.complete_refund(uuid,text),public.customer_timeline(uuid,integer,integer,text[]),public.create_import_batch(text,text,text,text,jsonb,jsonb),public.decide_import_row(uuid,text),public.process_import_batch(uuid,integer),public.rollback_import_batch(uuid),public.run_data_quality_rules(),public.resolve_data_quality_issue(uuid,text,boolean),public.create_appointment_with_delivery(text,text,text,text,uuid,text,timestamptz,timestamptz,text,integer[],jsonb),public.update_appointment_delivery(uuid,text,timestamptz,timestamptz) to crm_app;
revoke all on function public.claim_calendar_deliveries(integer),public.complete_calendar_delivery(uuid,text),public.fail_calendar_delivery(uuid,text) from public,crm_system,crm_app;
grant execute on function public.claim_calendar_deliveries(integer),public.complete_calendar_delivery(uuid,text),public.fail_calendar_delivery(uuid,text) to crm_system, crm_worker;
