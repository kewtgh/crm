...
from app_auth.accounts u cross join lateral (select coalesce(nullif(lower(raw_user_meta_data->>'username'), ''), regexp_replace(lower(split_part(email, '@', 1)), '[^a-z0-9._-]', '', 'g')) as candidate) names
