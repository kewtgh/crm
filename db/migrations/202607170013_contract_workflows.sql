...
begin
  ...
  values(organization.workspace_id,trim(contract_no),organization.id,target_product,period_start,period_end,contract_currency,contract_amount,'DRAFT',relationship,app_auth.current_user_id(),app_auth.current_user_id()) returning * into result;
  return result;
end; $$;
...
begin
  ...
  values(source.workspace_id,next_number,source.organization_id,source.product_id,next_start,next_end,source.currency,source.contract_value,'DRAFT',source.relationship_level,coalesce(source.owner_id,app_auth.current_user_id()),app_auth.current_user_id(),source.id) returning * into result;
...
grant execute on function public.create_contract_draft(text,uuid,uuid,date,date,text,numeric,smallint),public.create_contract_renewal(uuid) to crm_app;
