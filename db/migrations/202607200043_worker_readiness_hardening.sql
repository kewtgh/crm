...
grant execute on function public.service_readiness_snapshot(uuid,text[]) to crm_system, crm_worker;
...
begin
  ...
  values(ws,app_auth.current_user_id(),'operational_job',job_id::text,'RETRY',jsonb_build_object('type',normalized,'status',previous_status),jsonb_build_object('type',normalized,'status','REQUEUED','attempts',0),txid_current()::text);
end;
$$;
...
revoke all on function public.operational_retryable_jobs_page(integer,integer) from public,crm_system;
grant execute on function public.operational_retryable_jobs_page(integer,integer) to crm_app;
