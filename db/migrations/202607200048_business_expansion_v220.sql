...
create policy "automation leaders read rules" on public.automation_rules for select to crm_app
...
create policy "automation leaders create rules" on public.automation_rules for insert to crm_app
  with check(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
create policy "automation leaders update rules" on public.automation_rules for update to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'))
...
create policy "automation leaders read events" on public.automation_events for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
create policy "automation leaders read runs" on public.automation_runs for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
...
begin
  ...
  return public.dispatch_automation_event(public.current_workspace_id(),upper(target_trigger),target_event_key,target_payload,app_auth.current_user_id());
end;$$;
...
create or replace function public.opportunity_automation_trigger() returns trigger language plpgsql security definer set search_path=public,app_auth,extensions as $$
begin
...
create table public.growth_campaigns (
  ...
  starts_on date,ends_on date,created_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),
...
create table public.lead_attribution_touches (
  ...
  occurred_at timestamptz not null default now(),created_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),created_at timestamptz not null default now()
);
...
create table public.admission_journeys (
  ...
  owner_id uuid not null default app_auth.current_user_id() references app_auth.accounts(id),created_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),
...
create policy "sales read campaigns" on public.growth_campaigns for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "growth leaders manage campaigns" on public.growth_campaigns for all to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'))
  with check(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
create policy "sales read attribution" on public.lead_attribution_touches for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "sales record attribution" on public.lead_attribution_touches for insert to crm_app
  with check(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST'));
create policy "sales read journeys" on public.admission_journeys for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "sales create journeys" on public.admission_journeys for insert to crm_app
  with check(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and owner_id=app_auth.current_user_id() and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST'));
create policy "owners update journeys" on public.admission_journeys for update to crm_app
  using(public.is_workspace_member(workspace_id) and (owner_id=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER')))
  with check(public.is_workspace_member(workspace_id) and (owner_id=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER')));

create or replace function public.growth_snapshot()
returns jsonb language sql stable security definer set search_path=public,app_auth,extensions as $$
...
create table public.portal_invitations (
  ...
  expires_at timestamptz not null,last_accessed_at timestamptz,created_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),
...
create table public.portal_update_requests (
  ...
  decision_note text not null default '',decided_by uuid references app_auth.accounts(id),decided_at timestamptz,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),
...
create policy "portal staff read invitations" on public.portal_invitations for select to crm_app
...
create policy "portal staff create invitations" on public.portal_invitations for insert to crm_app
  with check(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id() and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST','SALES_SUPPORT'));
create policy "portal staff update invitations" on public.portal_invitations for update to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST','SALES_SUPPORT'))
...
create policy "portal staff read updates" on public.portal_update_requests for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER','SALES_SPECIALIST','SALES_SUPPORT'));
create policy "portal leaders decide updates" on public.portal_update_requests for update to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'))
  with check(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
...
begin
  if app_auth.current_db_role()<>'service_role' or target_digest!~'^[a-f0-9]{64}$' then raise exception 'portal_access_invalid'; end if;
...
returns uuid language plpgsql security definer set search_path=public,app_auth,extensions as $$
declare invitation public.portal_invitations;created_id uuid;
begin
  if app_auth.current_db_role()<>'service_role' or target_digest!~'^[a-f0-9]{64}$' or nullif(trim(target_request_key),'') is null
...
create table public.communication_threads (
  ...
  status text not null default 'OPEN' check(status in ('OPEN','CLOSED')),assigned_to uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
  created_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),last_message_at timestamptz,created_at timestamptz not null default now(),updated_at timestamptz not null default now()
);
...
create table public.communication_messages (
  ...
  provider_message_id text,last_error text,sent_by uuid references app_auth.accounts(id),created_at timestamptz not null default now(),delivered_at timestamptz
);
...
create policy "members read communication threads" on public.communication_threads for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "members read communication messages" on public.communication_messages for select to crm_app using(public.is_workspace_member(workspace_id));
...
returns public.communication_threads language plpgsql security definer set search_path=public,app_auth,extensions as $$
declare result public.communication_threads;ws uuid:=public.current_workspace_id();
begin
  if app_auth.current_user_id() is null or nullif(trim(target_subject),'') is null or upper(target_channel)<>'EMAIL'
  ...
  values(ws,target_contact,left(trim(target_subject),200),upper(target_channel),upper(target_purpose),app_auth.current_user_id(),app_auth.current_user_id()) returning * into result;
  return result;
end;$$;

create or replace function public.queue_communication_message(target_thread uuid,target_body text)
returns public.communication_messages language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
begin
  ...
  values(thread.workspace_id,thread.id,'OUTBOUND',left(trim(target_body),10000),'QUEUED',app_auth.current_user_id()) returning * into result;
...
returns void language plpgsql security definer set search_path=public,app_auth,extensions as $$
begin
  if app_auth.current_db_role()<>'service_role' then raise exception 'communication_service_forbidden'; end if;
...
create policy "quality leaders read trends" on public.data_quality_daily_snapshots for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
...
create or replace function public.data_quality_snapshot_trigger() returns trigger language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
returns public.integration_connections language plpgsql security definer set search_path=public,app_auth,extensions as $$
...
begin
  ...
  update public.integration_connections set status=normalized_status,sync_direction=upper(next_direction),external_account_label=trim(coalesce(account_label,'')),configured_by=app_auth.current_user_id(),last_error=null,updated_at=now()
...
grant select,insert,update on public.automation_rules to crm_app;
grant select on public.automation_events,public.automation_runs to crm_app;
grant select,insert,update on public.growth_campaigns,public.lead_attribution_touches,public.admission_journeys to crm_app;
grant select,insert,update on public.portal_invitations,public.portal_update_requests to crm_app;
grant select on public.communication_threads,public.communication_messages,public.data_quality_daily_snapshots to crm_app;
revoke all on function public.dispatch_automation_event(uuid,text,text,jsonb,uuid) from public,crm_system,crm_app;
revoke all on function public.run_automation_event(text,text,jsonb),public.growth_snapshot(),public.create_communication_thread(uuid,text,text,text),public.queue_communication_message(uuid,text) from public,crm_system;
grant execute on function public.run_automation_event(text,text,jsonb),public.growth_snapshot(),public.create_communication_thread(uuid,text,text,text),public.queue_communication_message(uuid,text) to crm_app;
revoke all on function public.service_portal_snapshot(text),public.service_submit_portal_update(text,text,jsonb),public.service_complete_communication(uuid,text),public.service_fail_communication(uuid,text) from public,crm_system,crm_app;
grant execute on function public.service_portal_snapshot(text),public.service_submit_portal_update(text,text,jsonb),public.service_complete_communication(uuid,text),public.service_fail_communication(uuid,text),public.confirm_integration_connection(uuid,text,text,text) to crm_system, crm_worker;
revoke all on function public.refresh_data_quality_daily_snapshot(uuid) from public,crm_system,crm_app;
