...
as $$ select upper(coalesce(app_auth.current_claims()->'app_metadata'->>'role', '')); $$;
...
create table if not exists public.approval_requests (
  ...
  requester_id uuid not null references app_auth.accounts(id),
...
create table if not exists public.approval_actions (
  ...
  actor_id uuid not null references app_auth.accounts(id),
...
create table if not exists public.performance_targets (
  id uuid primary key default gen_random_uuid(),
  manager_id uuid not null references app_auth.accounts(id),
...
create table if not exists public.performance_allocations (
  ...
  contributor_id uuid not null references app_auth.accounts(id),
...
create policy "approval participants can read" on public.approval_requests for select to crm_app
  using (requester_id = app_auth.current_user_id() or public.crm_role() in ('ADMIN', 'SUPER_ADMIN'));
create policy "crm_app users submit approvals" on public.approval_requests for insert to crm_app
  with check (requester_id = app_auth.current_user_id() and status = 'PENDING' and decided_by is null);
create policy "approval participants read audit" on public.approval_actions for select to crm_app
  using (exists (select 1 from public.approval_requests r where r.id = approval_request_id and (r.requester_id = app_auth.current_user_id() or public.crm_role() in ('ADMIN', 'SUPER_ADMIN'))));
create policy "participants append permitted audit" on public.approval_actions for insert to crm_app
  with check (actor_id = app_auth.current_user_id() and exists (
    select 1 from public.approval_requests r where r.id = approval_request_id and (
      (r.requester_id = app_auth.current_user_id() and action in ('SUBMITTED', 'CANCELLED', 'COMMENTED'))
      or (r.requester_id <> app_auth.current_user_id() and public.crm_role() in ('ADMIN', 'SUPER_ADMIN') and action = 'COMMENTED')
    )
  ));
...
create policy "sales hierarchy reads targets" on public.performance_targets for select to crm_app
  using (manager_id = app_auth.current_user_id() or public.crm_role() in ('SUPER_ADMIN', 'ADMIN', 'SALES_DIRECTOR'));
create policy "managers create targets" on public.performance_targets for insert to crm_app
  with check (created_by = app_auth.current_user_id() and public.crm_role() in ('SUPER_ADMIN', 'ADMIN', 'SALES_DIRECTOR', 'SALES_MANAGER'));
create policy "target owners update drafts" on public.performance_targets for update to crm_app
  using ((manager_id = app_auth.current_user_id() and status in ('DRAFT', 'PENDING_APPROVAL')) or public.crm_role() in ('SUPER_ADMIN', 'ADMIN', 'SALES_DIRECTOR'));
create policy "allocation participants read" on public.performance_allocations for select to crm_app
  using (contributor_id = app_auth.current_user_id() or exists (select 1 from public.performance_targets t where t.id = target_id and t.manager_id = app_auth.current_user_id()) or public.crm_role() in ('SUPER_ADMIN', 'ADMIN', 'SALES_DIRECTOR'));
create policy "target owners manage allocations" on public.performance_allocations for all to crm_app
  using (exists (select 1 from public.performance_targets t where t.id = target_id and (t.manager_id = app_auth.current_user_id() or public.crm_role() in ('SUPER_ADMIN', 'ADMIN', 'SALES_DIRECTOR'))))
  with check (created_by = app_auth.current_user_id() and exists (select 1 from public.performance_targets t where t.id = target_id and (t.manager_id = app_auth.current_user_id() or public.crm_role() in ('SUPER_ADMIN', 'ADMIN', 'SALES_DIRECTOR'))));

grant select, insert on public.approval_requests to crm_app;
grant select, insert on public.approval_actions to crm_app;
grant select, insert, update on public.performance_targets to crm_app;
grant select, insert, update, delete on public.performance_allocations to crm_app;
grant usage, select on sequence public.approval_actions_id_seq to crm_app;
