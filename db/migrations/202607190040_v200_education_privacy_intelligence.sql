...
returns text language sql stable security definer set search_path=public,app_auth,extensions
as $$
  select coalesce((
...
as $$
  select exists(
    ...
    where workspace_id=target_workspace and user_id=app_auth.current_user_id() and status='ACTIVE'
      and (
        upper(role) not in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER')
        or coalesce(app_auth.current_claims()->>'aal','aal1')='aal2'
      )
...
$$;
revoke all on function public.product_catalog_snapshot() from public,crm_system;
grant execute on function public.product_catalog_snapshot() to crm_app;
...
create table if not exists public.lead_conversions (
  ...
  idempotency_key text not null,
  converted_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
...
create table if not exists public.ai_suggestion_decisions (
  ...
  idempotency_key text not null,
  decided_by uuid not null default app_auth.current_user_id() references app_auth.accounts(id),
...
create policy "education members read households" on public.households for select to crm_app
  using(public.is_workspace_member(workspace_id));
create policy "education members manage households" on public.households for all to crm_app
...
create policy "education members read household members" on public.household_members for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "education members manage household members" on public.household_members for all to crm_app using(public.is_workspace_member(workspace_id)) with check(public.is_workspace_member(workspace_id));
create policy "education members read students" on public.students for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "education members manage students" on public.students for all to crm_app using(public.is_workspace_member(workspace_id)) with check(public.is_workspace_member(workspace_id));
create policy "education members read guardians" on public.student_guardian_relationships for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "education members manage guardians" on public.student_guardian_relationships for all to crm_app using(public.is_workspace_member(workspace_id)) with check(public.is_workspace_member(workspace_id));
create policy "education members read academics" on public.student_academic_records for select to crm_app using(public.is_workspace_member(workspace_id));
create policy "education members manage academics" on public.student_academic_records for all to crm_app using(public.is_workspace_member(workspace_id)) with check(public.is_workspace_member(workspace_id));
create policy "leaders manage progression" on public.progression_batches for all to crm_app
...
create policy "leaders manage progression items" on public.progression_batch_items for all to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'))
  with check(public.is_workspace_member(workspace_id) and public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR','SALES_MANAGER'));
create policy "sales read leads" on public.leads for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role()<>'SALES_SUPPORT');
create policy "sales manage leads" on public.leads for all to crm_app
...
create policy "sales read lead conversions" on public.lead_conversions for select to crm_app
  using(public.is_workspace_member(workspace_id) and public.current_crm_role()<>'SALES_SUPPORT');
create policy "privacy owners read requests" on public.privacy_requests for select to crm_app
  using(public.is_workspace_member(workspace_id) and (created_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN','SALES_DIRECTOR')));
create policy "members create privacy requests" on public.privacy_requests for insert to crm_app
  with check(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id());
create policy "privacy leaders manage requests" on public.privacy_requests for update to crm_app
...
create policy "owners manage mappings" on public.import_mapping_profiles for all to crm_app
  using(public.is_workspace_member(workspace_id) and (owned_by=app_auth.current_user_id() or public.current_crm_role() in ('SUPER_ADMIN','ADMIN')))
  with check(public.is_workspace_member(workspace_id) and owned_by=app_auth.current_user_id());
create policy "members read own suggestion runs" on public.ai_suggestion_runs for select to crm_app
  using(public.is_workspace_member(workspace_id) and created_by=app_auth.current_user_id());
create policy "members read suggestions" on public.ai_suggestions for select to crm_app
  using(public.is_workspace_member(workspace_id) and exists(select 1 from public.ai_suggestion_runs r where r.id=run_id and r.created_by=app_auth.current_user_id()));
create policy "members read suggestion decisions" on public.ai_suggestion_decisions for select to crm_app
  using(public.is_workspace_member(workspace_id) and decided_by=app_auth.current_user_id());
...
begin
  ...
  values(lead.workspace_id,lead.organization_id,trim(title_zh),trim(title_en),amount,upper(currency),lead.owner_id,app_auth.current_user_id())
  returning * into result;
...
begin
  ...
  select s.* into suggestion from public.ai_suggestions s
    ...
    where s.id=target_suggestion and s.workspace_id=public.current_workspace_id() and r.created_by=app_auth.current_user_id() for update;
  ...
  if create_task and decision_value in ('ACCEPTED','EDITED') then
    ...
    values(suggestion.workspace_id,coalesce(nullif(trim(final_zh),''),suggestion.recommendation_zh),coalesce(nullif(trim(final_en),''),suggestion.recommendation_en),suggestion.subject_type,suggestion.subject_id,'TODO','NORMAL',app_auth.current_user_id())
...
begin
  ...
  if request_row.request_type in ('EXPORT','DELETION') then
    ...
    if request_row.status='IN_PROGRESS' and normalized_status='WAITING_APPROVAL' then
      insert into public.crm_tasks(
        ...
        'WAITING_APPROVAL','URGENT',null,request_row.due_at,app_auth.current_user_id()
      ) returning id into created_task;
    elsif request_row.status='WAITING_APPROVAL' and normalized_status='FULFILLED' then
      if request_row.assigned_to=app_auth.current_user_id() then
      ...
      update public.crm_tasks set
        status='DONE',completed_at=now(),updated_at=now(),owner_id=app_auth.current_user_id()
  ...
  update public.privacy_requests set
    ...
    assigned_to=case
      when normalized_status='WAITING_APPROVAL' then app_auth.current_user_id()
      when normalized_status in ('FULFILLED','REJECTED','CANCELLED') then assigned_to
      else app_auth.current_user_id()
    end,
...
begin
  ...
  values(public.current_workspace_id(),upper(resource),left(filename,180),content_hash,request_key,mapping,app_auth.current_user_id())
...
begin
  ...
  insert into public.approval_requests(
    ...
    resource_key||':'||gen_random_uuid(),app_auth.current_user_id(),trim(business_reason),now()+interval '7 days',
    jsonb_build_object(
      ...
      'scope',scope_value,'requesterId',app_auth.current_user_id(),'format',normalized_format
    )
  ) returning * into created;
  insert into public.approval_actions(approval_request_id,actor_id,action,comment)
    values(created.id,app_auth.current_user_id(),'SUBMITTED',trim(business_reason));
  return created;
...
begin
  ...
  if batch_record.created_by<>app_auth.current_user_id()
