...
export async function loadOrganization360(id:string,page=1,pageSize=20,types:string[]=[]):Promise<Organization360>{
  const organizations=await databaseJson<Array<{id:string;name_zh:string;name_en:string;status:string;city:string;curriculum:string;completeness:number}>>(`/db/table/organizations?select=id,name_zh,name_en,status,city,curriculum,completeness&id=eq.${id}&limit=1`);
  if(!organizations[0])throw new Error("ORGANIZATION_NOT_FOUND");
  const timeline=await databaseJson<TimelinePage>("/db/rpc/customer_timeline",{method:"POST",body:JSON.stringify({target_organization:id,page_number:page,page_size:pageSize,event_types:types.length?types:null})});
...
export type ContactPrivacy={id:string;nameZh:string;nameEn:string;email:string;phone:string;title:string;contactType:string;recordStatus:string;contactStatus:string;communicationLevel:number;notesMarkdown:string;ownerId:string|null;ownerName:string;preferredContactMethod:string;preferredLanguage:string;acquisitionSource:string;decisionRole:string;tags:string[];nextFollowUpAt:string|null;households:Array<{id:string;nameZh:string;nameEn:string;role:string;primary:boolean}>;doNotContact:boolean;doNotContactReason:string;consents:ConsentRecord[]};
export async function loadContactPrivacy(id:string):Promise<ContactPrivacy>{
  const contacts=await databaseJson<Array<{id:string;name_zh:string;name_en:string;email:string|null;phone:string|null;title:string;contact_type:string;status:string;contact_status:string;communication_level:number;notes_markdown:string;owner_id:string|null;preferred_contact_method:string;preferred_language:string;acquisition_source:string;decision_role:string;tags:string[];next_follow_up_at:string|null;do_not_contact:boolean;do_not_contact_reason:string}>>(`/db/table/contacts?select=id,name_zh,name_en,email,phone,title,contact_type,status,contact_status,communication_level,notes_markdown,owner_id,preferred_contact_method,preferred_language,acquisition_source,decision_role,tags,next_follow_up_at,do_not_contact,do_not_contact_reason&id=eq.${id}&limit=1`);const contact=contacts[0];if(!contact)throw new Error("CONTACT_NOT_FOUND");
  const [consents,memberships,owners]=await Promise.all([databaseJson<Array<{id:string;channel:string;purpose:string;status:string;source:string;evidence_note:string;obtained_at:string|null;revoked_at:string|null;retention_until:string|null;quiet_hours_start:string|null;quiet_hours_end:string|null}>>(`/db/table/contact_consents?select=*&contact_id=eq.${id}&order=channel,purpose`),databaseJson<Array<{member_role:string;primary_contact:boolean;households:{id:string;name_zh:string;name_en:string}|null}>>(`/db/table/household_members?select=member_role,primary_contact,households:households!household_members_household_id_fkey(id,name_zh,name_en)&contact_id=eq.${id}`),contact.owner_id?databaseJson<Array<{display_name_zh:string;display_name_en:string}>>(`/db/table/user_profiles?select=display_name_zh,display_name_en&user_id=eq.${contact.owner_id}&limit=1`):Promise.resolve([])]);
  const owner=owners[0];return{id:contact.id,nameZh:contact.name_zh,nameEn:contact.name_en,email:contact.email??"",phone:contact.phone??"",title:contact.title,contactType:contact.contact_type,recordStatus:contact.status,contactStatus:contact.contact_status,communicationLevel:Number(contact.communication_level),notesMarkdown:contact.notes_markdown,ownerId:contact.owner_id,ownerName:owner?`${owner.display_name_zh} / ${owner.display_name_en}`:"—",preferredContactMethod:contact.preferred_contact_method,preferredLanguage:contact.preferred_language,acquisitionSource:contact.acquisition_source,decisionRole:contact.decision_role,tags:contact.tags??[],nextFollowUpAt:contact.next_follow_up_at,households:memberships.flatMap(item=>item.households?[{id:item.households.id,nameZh:item.households.name_zh,nameEn:item.households.name_en,role:item.member_role,primary:item.primary_contact}]:[]),doNotContact:contact.do_not_contact,doNotContactReason:contact.do_not_contact_reason,consents:consents.map(item=>({id:item.id,channel:item.channel,purpose:item.purpose,status:item.status,source:item.source,evidenceNote:item.evidence_note,obtainedAt:item.obtained_at,revokedAt:item.revoked_at,retentionUntil:item.retention_until,quietStart:item.quiet_hours_start,quietEnd:item.quiet_hours_end}))};
}
...
async function fetchFinancePage<T>(path:string,page:number,pageSize:number){
  const response=await databaseRequest(path,{headers:{Prefer:"count=exact",Range:`${(page-1)*pageSize}-${page*pageSize-1}`}});
...
async function countFinanceRows(path:string){
  const response=await databaseRequest(path,{headers:{Prefer:"count=exact",Range:"0-0"}});
  await response.json();
...
export async function importOperation(input:Record<string,unknown>){const operation=String(input.operation);const rpc=operation==="create"?"create_import_batch":operation==="decide"?"decide_import_row":operation==="repair"?"repair_import_row":operation==="process"?"process_import_batch":operation==="rollback"?"idempotent_rollback_import_batch":"";if(!rpc)throw new Error("INVALID_IMPORT_OPERATION");const {operation:_,requestKey,...body}=input;void _;if(operation==="rollback")body.p_request_key=requestKey;const result=await databaseJson<Record<string,unknown>>(`/db/rpc/${rpc}`,{method:"POST",body:JSON.stringify(body)});return operation==="decide"||operation==="repair"?result:mapImportBatch(result);}
export async function listImportMappingProfiles(resource?:string){const filter=resource?`&resource=eq.${encodeURIComponent(resource)}`:"";const rows=await databaseJson<Array<{id:string;resource:ImportMappingProfile["resource"];name:string;mapping:Record<string,string>;updated_at:string}>>(`/db/table/import_mapping_profiles?select=id,resource,name,mapping,updated_at${filter}&order=name`);return{items:rows.map(item=>({id:item.id,resource:item.resource,name:item.name,mapping:item.mapping,updatedAt:item.updated_at} satisfies ImportMappingProfile))};}
export async function saveImportMappingProfile(input:{resource:ImportMappingProfile["resource"];name:string;mapping:Record<string,string>}){const rows=await databaseJson<Array<{id:string;resource:ImportMappingProfile["resource"];name:string;mapping:Record<string,string>;updated_at:string}>>("/db/table/import_mapping_profiles?on_conflict=workspace_id,owned_by,resource,name",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify({resource:input.resource,name:input.name,mapping:input.mapping,updated_at:new Date().toISOString()})});const item=rows[0];if(!item)throw new Error("IMPORT_MAPPING_SAVE_FAILED");return{id:item.id,resource:item.resource,name:item.name,mapping:item.mapping,updatedAt:item.updated_at} satisfies ImportMappingProfile;}
...
export async function listQualityIssues(options:{query?:string;page?:number;pageSize?:number;status?:string}={}){const page=Math.max(1,options.page??1),pageSize=Math.min(50,options.pageSize??10),start=(page-1)*pageSize;const params=new URLSearchParams({select:"*",order:"last_seen_at.desc"});if(options.status&&options.status!=="all")params.set("status",`eq.${options.status}`);const query=(options.query??"").replace(/[*,()]/g," ").trim();if(query)params.set("or",`(rule_key.ilike.*${query}*,entity_type.ilike.*${query}*)`);const response=await databaseRequest(`/db/table/data_quality_issues?${params}`,{headers:{Prefer:"count=exact",Range:`${start}-${start+pageSize-1}`}});const rows=await response.json() as Array<Record<string,unknown>>;return{items:rows.map(item=>({id:String(item.id),ruleKey:String(item.rule_key),entityType:String(item.entity_type),entityId:String(item.entity_id),severity:String(item.severity),titleKey:String(item.title_key),details:item.details as Record<string,string>,status:String(item.status),assignedTo:item.assigned_to?String(item.assigned_to):null,resolution:String(item.resolution_note??""),lastSeenAt:String(item.last_seen_at)} satisfies QualityIssue)),total:Number((response.headers.get("content-range")??"*/0").split("/")[1]??rows.length)};}
export async function qualityOperation(input:{operation:"run"|"resolve";id?:string;resolution?:string;dismiss?:boolean}){return input.operation==="run"?databaseJson<number>("/db/rpc/run_data_quality_rules",{method:"POST",body:"{}"}):databaseJson("/db/rpc/resolve_data_quality_issue",{method:"POST",body:JSON.stringify({target_issue:input.id,resolution:input.resolution,dismiss:input.dismiss??false})});}
...
export async function listCalendarDeliveries(appointmentId:string){const rows=await databaseJson<Array<{id:string;appointment_id:string;attendee_id:string;delivery_type:string;event_version:number;status:string;attempts:number;last_error:string|null;delivered_at:string|null}>>(`/db/table/calendar_deliveries?select=id,appointment_id,attendee_id,delivery_type,event_version,status,attempts,last_error,delivered_at&appointment_id=eq.${appointmentId}&order=created_at.desc`);const ids=[...new Set(rows.map(item=>item.attendee_id))];const attendees=ids.length?await databaseJson<Array<{id:string;email:string;name:string}>>(`/db/table/appointment_attendees?select=id,email,name&id=in.(${ids.join(",")})`):[];const map=new Map(attendees.map(item=>[item.id,item]));return rows.map(item=>({id:item.id,appointmentId:item.appointment_id,email:map.get(item.attendee_id)?.email??"",name:map.get(item.attendee_id)?.name??"",type:item.delivery_type,version:item.event_version,status:item.status,attempts:item.attempts,lastError:item.last_error??"",deliveredAt:item.delivered_at}));}
