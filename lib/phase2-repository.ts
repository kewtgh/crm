...
export async function loadContactPrivacy(id:string):Promise<ContactPrivacy>{
  const contacts=await databaseJson<Array<{id:string;name_zh:string;name_en:string;email:string|null;phone:string|null;title:string;do_not_contact:boolean;do_not_contact_reason:string}>>(`/db/table/contacts?select=id,name_zh,name_en,email,phone,title,do_not_contact,do_not_contact_reason&id=eq.${id}&limit=1`);const contact=contacts[0];if(!contact)throw new Error("CONTACT_NOT_FOUND");
  const consents=await databaseJson<Array<{id:string;channel:string;purpose:string;status:string;source:string;evidence_note:string;obtained_at:string|null;revoked_at:string|null;retention_until:string|null;quiet_hours_start:string|null;quiet_hours_end:string|null}>>(`/db/table/contact_consents?select=*&contact_id=eq.${id}&order=channel,purpose`);
...
export type ImportMappingProfile={id:string;resource:"CONTACTS"|"ORGANIZATIONS";name:string;mapping:Record<string,string>;updatedAt:string};
