...
export async function listProducts():Promise<ProductRecord[]>{
  ...
  return products.map((item)=>{
    ...
    return {
      ...
      active:Boolean(item.active),lifecycleStatus:String(item.lifecycleStatus??(item.active?"ACTIVE":"PAUSED")) as ProductLifecycleStatus,isDefault:Boolean(item.isDefault),currency:primary.currency,updatedAt:String(item.updatedAt),
    };
  });
}

export async function createProduct(input:{nameZh:string;nameEn:string;code:string;descriptionZhMarkdown:string;descriptionEnMarkdown:string;lifecycleStatus:ProductLifecycleStatus;price:number;currency:string;billing:string;duration:string;durationEn:string}){return databaseJson<Record<string,unknown>>("/db/rpc/create_product_with_price",{method:"POST",body:JSON.stringify({product_code:input.code,product_name_zh:input.nameZh,product_name_en:input.nameEn,product_billing:input.billing,product_duration_zh:input.duration,product_duration_en:input.durationEn,product_description_zh_markdown:input.descriptionZhMarkdown,product_description_en_markdown:input.descriptionEnMarkdown,product_lifecycle_status:input.lifecycleStatus,price_currency:input.currency,price_amount:input.price})});}
export async function setProductLifecycle(id:string,lifecycleStatus:ProductLifecycleStatus,requestKey:string){return databaseJson<Record<string,unknown>>("/db/rpc/idempotent_set_product_lifecycle",{method:"POST",body:JSON.stringify({target_product:id,target_status:lifecycleStatus,p_request_key:requestKey})});}
...
export async function updateProduct(input:{id:string;expectedUpdatedAt:string;nameZh:string;nameEn:string;code:string;descriptionZhMarkdown:string;descriptionEnMarkdown:string;lifecycleStatus:ProductLifecycleStatus;billing:string;duration:string;durationEn:string;isDefault:boolean}){return databaseJson<Record<string,unknown>>("/db/rpc/update_product_record",{method:"POST",body:JSON.stringify({target_product:input.id,expected_updated_at:input.expectedUpdatedAt,product_code:input.code,product_name_zh:input.nameZh,product_name_en:input.nameEn,product_billing:input.billing,product_duration_zh:input.duration,product_duration_en:input.durationEn,product_description_zh_markdown:input.descriptionZhMarkdown,product_description_en_markdown:input.descriptionEnMarkdown,product_lifecycle_status:input.lifecycleStatus,product_is_default:input.isDefault})});}
