...
export async function listNotifications(page=1,pageSize=10){
  const response=await databaseRequest("/db/table/user_notifications?select=id,kind,title_key,body_key,values,source_type,source_id,created_at&read_at=is.null&order=created_at.desc",{headers:{Prefer:"count=exact",Range:`${(Math.max(1,page)-1)*pageSize}-${Math.max(1,page)*pageSize-1}`}});
...
export async function markNotificationsRead(ids?:string[]){
  ...
  await databaseJson(`/db/table/user_notifications?${filter}`,{method:"PATCH",body:JSON.stringify({read_at:new Date().toISOString()}),headers:{Prefer:"return=minimal"}});
}
