...
export async function loadSalesPerformance(period:"month"|"quarter"|"year"="quarter",team="all",currency?:string|null):Promise<SalesPerformanceData>{
  const raw=await databaseJson<Record<string,unknown>>("/db/rpc/sales_performance_report_v220",{method:"POST",body:JSON.stringify({report_period:period,team_filter:team,currency_filter:currency?.toUpperCase()??null})});
...
export async function recordRelationshipMilestone(input:{organizationId:string;milestone:"CONTACT"|"MEAL"|"FAMILY_CHAT"|"ADVOCACY";evidence:string}){
  return databaseJson<Record<string,unknown>>("/db/rpc/upsert_relationship_milestone",{method:"POST",body:JSON.stringify({target_organization:input.organizationId,milestone:input.milestone,evidence:input.evidence})});
}
...
export async function updateOpportunity(id:string,input:{stage:OpportunityRecord["stage"];probability:number;expectedCloseDate?:string|null;nextActionZh:string;nextActionEn:string;reason?:string;evidence?:string}){
  return databaseJson<OpportunityRow>("/db/rpc/change_opportunity_stage",{
    method:"POST",
    body:JSON.stringify({
