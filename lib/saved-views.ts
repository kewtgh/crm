...
export async function listSharedViews(resource:string,currentUserId:string){
  const rows=await databaseJson<SharedViewRow[]>(`/db/table/shared_views?select=id,name,visibility,config,owner_id&resource_key=eq.${encodeURIComponent(resource)}&order=updated_at.desc`);
...
export async function deleteSharedView(id:string){
  return databaseJson("/db/rpc/delete_shared_view",{method:"POST",body:JSON.stringify({target_view:id})});
}
