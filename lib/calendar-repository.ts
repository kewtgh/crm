...
export async function changeAppointmentDelivery(id:string,action:"UPDATE"|"CANCEL",date?:string,time?:string,timezone:SupportedTimezone=normalizeTimezone(undefined)){let starts:string|null=null,ends:string|null=null;if(action==="UPDATE"&&date&&time){const start=zonedLocalDateTimeToUtc(`${date}T${time}`,timezone);starts=start.toISOString();ends=new Date(start.getTime()+3600000).toISOString();}return supabaseJson("/rest/v1/rpc/update_appointment_delivery",{method:"POST",body:JSON.stringify({target_appointment:id,action,starts,ends})});}
