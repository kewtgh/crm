...
export async function listAdminAudits(options:{query?:string;page?:number;pageSize?:number}={}):Promise<AdminAuditPage>{
  ...
  const response=await databaseRequest(`/db/table/audit_events?${params}`,{headers:{Prefer:"count=exact",Range:`${start}-${start+pageSize-1}`}});const audits=await response.json() as Array<{id:string;action:string;entity_type:string;entity_id:string|null;actor_id:string|null;created_at:string}>;const total=Number((response.headers.get("content-range")??"*/0").split("/")[1]??audits.length);
  const actorIds=[...new Set(audits.map(item=>item.actor_id).filter(Boolean))] as string[];const names=new Map<string,{zh:string;en:string}>();if(actorIds.length){const profiles=await databaseJson<Array<{user_id:string;display_name_zh:string;display_name_en:string}>>(`/db/table/user_profiles?select=user_id,display_name_zh,display_name_en&user_id=in.(${actorIds.join(",")})`);profiles.forEach(profile=>names.set(profile.user_id,{zh:profile.display_name_zh,en:profile.display_name_en}));}
...
export async function loadAdminDashboard():Promise<AdminDashboardData>{
  const [metrics,audits]=await Promise.all([databaseJson<Record<string,number>>("/db/rpc/admin_dashboard_metrics",{method:"POST",body:"{}"}),listAdminAudits({page:1,pageSize:8})]);
