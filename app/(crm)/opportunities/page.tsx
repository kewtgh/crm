...
export default async function Page(){let data;try{data=await Promise.all([listOpportunities({page:1,pageSize:24}),loadSalesPerformance("quarter","all")]);}catch{return <DataLoadError detailKey="pipeline.loadFailed"/>;}const [opportunities,performance]=data;return <PipelinePage initialItems={opportunities.items} initialTotal={opportunities.total} initialFunnel={performance.funnel}/>;}
