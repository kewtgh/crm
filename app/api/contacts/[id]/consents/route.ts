...
async function get(_:Request,context:{params:Promise<{id:string}>}){await requireApiUser();const id=parseUuid((await context.params).id);try{return NextResponse.json(await loadContactPrivacy(id));}catch{return NextResponse.json({code:"CONTACT_PRIVACY_LOAD_FAILED"},{status:500});}}
