...
const importField=z.enum(["nameZh","nameEn","email","phone","city","title"]);
...
const create=z.object({operation:z.literal("create"),resource:z.enum(["ORGANIZATIONS","CONTACTS"]),filename:z.string().trim().min(1).max(180),content_hash:z.string().regex(/^[a-f0-9]{64}$/i),request_key:z.string().min(8).max(160),mapping:mappingSchema,rows:z.array(importRow).min(1).max(IMPORT_MAX_ROWS)});
...
const saveMapping=z.object({operation:z.literal("saveMapping"),resource:z.enum(["ORGANIZATIONS","CONTACTS"]),name:z.string().trim().min(1).max(80),mapping:mappingSchema});
