...
async function get(request:Request){await requireApiRole("SUPER_ADMIN","ADMIN","SALES_DIRECTOR","SALES_MANAGER");const url=new URL(request.url);const{page,pageSize}=parsePagination(url.searchParams,15);try{return NextResponse.json(await listQualityIssues({query:url.searchParams.get("q")??"",page,pageSize,status:url.searchParams.get("status")??"all"}));}catch{return NextResponse.json({code:"QUALITY_LOAD_FAILED"},{status:500});}}
