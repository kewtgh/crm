...
async function get(request:Request){await requireApiUser();const url=new URL(request.url);const{page,pageSize}=parsePagination(url.searchParams,20);try{const [opportunities,performance]=await Promise.all([listOpportunities({page,pageSize,query:url.searchParams.get("query")??"",stage:url.searchParams.get("stage")??"all"}),loadSalesPerformance("quarter","all")]);return NextResponse.json({...opportunities,funnel:performance.funnel});}catch(error){return fail(error);}}
