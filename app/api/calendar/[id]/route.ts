...
async function patch(request: Request, context: { params: Promise<{ id: string }> }) { if(!mutationIsTrusted(request))return NextResponse.json({code:"UNTRUSTED_ORIGIN"},{status:403}); const user=await requireApiUser(); const id=parseUuid((await context.params).id);const body=await request.json().catch(()=>({action:"COMPLETE"}));const parsed=schema.safeParse(body);if(!parsed.success)return NextResponse.json({code:"INVALID_CALENDAR_ACTION"},{status:400}); try {if(parsed.data.action==="COMPLETE")await completeAppointment(id);else{const settings=await loadUserSettings(user);await changeAppointmentDelivery(id,parsed.data.action,parsed.data.action==="UPDATE"?parsed.data.date:undefined,parsed.data.action==="UPDATE"?parsed.data.time:undefined,settings.timezone);} return NextResponse.json({ ok: true }); } catch { return NextResponse.json({ code: "CALENDAR_UPDATE_FAILED" }, { status: 500 }); } }
