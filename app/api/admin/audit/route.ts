...
async function get(request:Request){
  ...
  catch(error){return error instanceof DatabaseRequestError?NextResponse.json({code:error.code},{status:error.status}):NextResponse.json({code:"AUDIT_LOAD_FAILED"},{status:500});}
}
