...
async function get(request:Request){
  await requireApiUser();const url=new URL(request.url);const value=url.searchParams.get("period");const period=value==="month"||value==="year"?value:"quarter";const team=(url.searchParams.get("team")??"all").slice(0,80);
