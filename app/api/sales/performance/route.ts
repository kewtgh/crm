...
async function get(request:Request){
  ...
  try{return NextResponse.json({data:await loadSalesPerformance(period,team,currency)});}catch(error){return NextResponse.json({code:error instanceof DatabaseRequestError?error.code:"SALES_REPORT_FAILED"},{status:error instanceof DatabaseRequestError?error.status:500});}
}
