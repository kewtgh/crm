...
const fail=(error:unknown)=>error instanceof DatabaseRequestError?NextResponse.json({code:error.code},{status:error.status}):NextResponse.json({code:"RELATIONSHIP_SAVE_FAILED"},{status:500});
