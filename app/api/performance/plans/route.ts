...
async function post(request:Request){if(!mutationIsTrusted(request))return NextResponse.json({code:"UNTRUSTED_ORIGIN"},{status:403});const parsed=schema.safeParse(await request.json().catch(()=>({})));if(!parsed.success)return NextResponse.json({code:"INVALID_INPUT",field:String(parsed.error.issues[0]?.path[0]??"form")},{status:400});const user=await requireApiRole("SUPER_ADMIN","ADMIN","SALES_DIRECTOR","SALES_MANAGER");await requireApiAal2();try{const item=parsed.data.operation==="save"?await savePerformanceWorkspace(parsed.data):await submitPerformanceWorkspace(parsed.data.targetId,parsed.data.reason);if(parsed.data.operation==="submit"&&["SUPER_ADMIN","ADMIN"].includes(user.role)){const id=approvalRecordId(item);if(!id)return NextResponse.json({code:"APPROVAL_ID_MISSING"},{status:500});return NextResponse.json({item:await executeSuperAdminApproval(id),direct:true});}return NextResponse.json({item,direct:false});}catch(error){return error instanceof DatabaseRequestError?NextResponse.json({code:error.code,message:error.message},{status:error.status}):NextResponse.json({code:"PERFORMANCE_SAVE_FAILED"},{status:500});}}
