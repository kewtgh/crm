...
export async function POST(request:Request){const parsed=schema.safeParse(await request.json().catch(()=>({})));if(!parsed.success)return NextResponse.json({code:"INVALID_INPUT",field:String(parsed.error.issues[0]?.path[0]??"form")},{status:400});try{const item=parsed.data.operation==="save"?await savePerformanceWorkspace(parsed.data):await submitPerformanceWorkspace(parsed.data.targetId,parsed.data.reason);return NextResponse.json({item});}catch(error){return error instanceof SupabaseRequestError?NextResponse.json({code:error.code,message:error.message},{status:error.status}):NextResponse.json({code:"PERFORMANCE_SAVE_FAILED"},{status:500});}}
