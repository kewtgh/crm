...
export async function GET(request: Request, context: { params: Promise<{ resource: string }> }) {
  ...
  try {
    ...
    if (url.searchParams.get("format") === "csv") {
      const lines = [["id","primary","secondary","status","meta","extra"], ...result.items.map((item) => [item.id,item.primary,item.secondary,item.status,item.meta,item.extra])];
      const csv = lines.map((line) => line.map((value) => `"${String(value).replaceAll('"','""')}"`).join(",")).join("\r\n");
      return new Response(`\uFEFF${csv}`, { headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": `attachment; filename="${resource}.csv"`, "cache-control": "no-store" } });
    }
...
export async function POST(request: Request, context: { params: Promise<{ resource: string }> }) {
  ...
  try {
    ...
    const item = await createCrmRecord(resource, parsed.data);
