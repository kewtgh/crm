...
async function patch(request:Request,context:{params:Promise<{id:string}>}){if(!mutationIsTrusted(request))return NextResponse.json({code:"UNTRUSTED_ORIGIN"},{status:403});await requireApiCapability("tasks.manage");const id=parseUuid((await context.params).id);try{await completeDashboardTask(id);return NextResponse.json({ok:true});}catch(error){return error instanceof DatabaseRequestError?NextResponse.json({code:error.code},{status:error.status}):NextResponse.json({code:"TASK_UPDATE_FAILED"},{status:500});}}
