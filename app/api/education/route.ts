...
const schema = z.discriminatedUnion("operation", [
  z.object({ operation: z.literal("createHousehold"), nameZh: z.string().trim().min(1).max(120), nameEn: z.string().trim().min(1).max(160), address: z.string().trim().max(1000).default(""),primaryParentOccupation:z.string().trim().max(160).default(""),secondaryParentOccupation:z.string().trim().max(160).default(""),annualIncomeAmount:z.number().nonnegative().nullable().optional(),incomeCurrency:z.string().regex(/^[A-Z]{3}$/).default("CNY"),preferredContactMethod:z.enum(["EMAIL","PHONE","SMS","WECHAT","WHATSAPP","IN_PERSON"]).default("EMAIL"),preferredLanguage:z.string().trim().max(80).default(""),educationExpectationsMarkdown:z.string().max(10000).default(""),familyBackgroundMarkdown:z.string().max(10000).default("") }),
  z.object({ operation: z.literal("createStudent"), personId: z.uuid(), householdId: z.uuid().nullable().optional(), studentNumber: z.string().trim().max(60).optional(),birthDate:z.iso.date().nullable().optional(), grade: z.string().trim().min(1).max(40),currentClass:z.string().trim().max(80).default(""), academicYear: z.string().trim().min(4).max(20),personalityMarkdown:z.string().max(10000).default(""),learningExpectationsMarkdown:z.string().max(10000).default(""),strengthsMarkdown:z.string().max(10000).default(""),supportNeedsMarkdown:z.string().max(10000).default(""),interests:z.array(z.string().trim().min(1).max(80)).max(30).default([]),preferredLearningStyle:z.enum(["UNSPECIFIED","VISUAL","AUDITORY","READ_WRITE","KINESTHETIC","MIXED"]).default("UNSPECIFIED") }),
  z.object({ operation: z.literal("updateStudent"), id: z.uuid(), expectedUpdatedAt: z.iso.datetime(), householdId: z.uuid().nullable().optional(),studentNumber:z.string().trim().max(60).default(""),birthDate:z.iso.date().nullable().optional(), grade: z.string().trim().min(1).max(40),currentClass:z.string().trim().max(80).default(""), academicYear: z.string().trim().min(4).max(20), status: z.enum(["ACTIVE","ON_LEAVE","ALUMNI","WITHDRAWN","ARCHIVED"]),personalityMarkdown:z.string().max(10000).default(""),learningExpectationsMarkdown:z.string().max(10000).default(""),strengthsMarkdown:z.string().max(10000).default(""),supportNeedsMarkdown:z.string().max(10000).default(""),interests:z.array(z.string().trim().min(1).max(80)).max(30).default([]),preferredLearningStyle:z.enum(["UNSPECIFIED","VISUAL","AUDITORY","READ_WRITE","KINESTHETIC","MIXED"]).default("UNSPECIFIED") }),
  z.object({ operation: z.literal("updateHousehold"), id: z.uuid(), expectedUpdatedAt: z.iso.datetime(), nameZh: z.string().trim().min(1).max(120), nameEn: z.string().trim().min(1).max(160), address: z.string().trim().max(1000), status: z.enum(["ACTIVE","INACTIVE","ARCHIVED"]),primaryParentOccupation:z.string().trim().max(160).default(""),secondaryParentOccupation:z.string().trim().max(160).default(""),annualIncomeAmount:z.number().nonnegative().nullable().optional(),incomeCurrency:z.string().regex(/^[A-Z]{3}$/).default("CNY"),preferredContactMethod:z.enum(["EMAIL","PHONE","SMS","WECHAT","WHATSAPP","IN_PERSON"]).default("EMAIL"),preferredLanguage:z.string().trim().max(80).default(""),educationExpectationsMarkdown:z.string().max(10000).default(""),familyBackgroundMarkdown:z.string().max(10000).default("") }),
