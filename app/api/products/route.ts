...
const toggleSchema=z.object({operation:z.literal("toggle"),id:z.string().uuid(),active:z.boolean()});const priceSchema=z.object({operation:z.literal("price"),id:z.string().uuid(),currency:z.string().regex(/^[A-Z]{3}$/),amount:z.number().nonnegative(),effectiveOn:z.string().date()});const schema=z.discriminatedUnion("operation",[createSchema,toggleSchema,priceSchema]);
