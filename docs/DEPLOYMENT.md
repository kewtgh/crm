# Lumina CRM 部署指引（v0.4.1）

## 部署边界

v0.4.1 可以部署为受限的 UI/认证验收环境，但**不应作为真实客户数据的生产 CRM**。审批与业绩分配表已经提供迁移和 RLS 基线，其余业务表、工作区 RLS、持久化设置、日程通知、MFA 管理和服务端分页完成前，不要导入学生、家庭或学校真实资料。
...
本地 Chromium 1228 核心回归已完成。正式上线仍必须先完成 `ARCHITECTURE_UI_REVIEW.md` 中的 P0 项：应用数据库与 workspace RLS、服务端分页、审批/分配事务、真实设置/MFA、权限矩阵，并在正式域名上复验 Turnstile。未完成时仅可用于内部 UI/认证验收。
